...
- **Contacts & Dialer**: address book with two-way Google Contacts sync (People API, incremental); number-pad dialer, phone→biolink resolution via `linked_identifiers`, silent biolink auto-attach (with detach memory). Bulk CSV/vCard import is a parse→preview→confirm flow (skip rows before commit; large files processed via background job with progress + plan-cap checks). Scheduled `contacts:sync` every 30 min; `tel:`/`mailto:` only. **Universal finder**: the dialer search is a grouped universal finder (`DialerSearch::universal`) matching contact names/orgs, user display names/handles, biolink aliases/back-halves, link/biolink SEO meta (title/desc/keywords), workspace names and verification status across owned AND followed records (visibility-gated); results grouped by category (Contacts, People, My links, Followed, Workspaces) with type label + action target, plus advanced contacts search (`contactsAdvanced`) and verified / on-Sayzio filter chips. T9 smart-dial preserved; both web and mobile expose a keypad toggle (T9 digit grid ↔ alphanumeric keyboard) that feed the same search. REST parity at `/api/v1/dialer/search`.
...
- **Production scheduler/queue (Replit)**: the production run command backgrounds a supervised `php artisan schedule:work` loop alongside the web server, so all `routes/schedules/*.php` jobs tick on the published app — including the every-minute `queue:work --stop-when-empty` drain that delivers queued emails/notifications. Scheduler output goes to `storage/logs/scheduler.log`; a `::1inme:: schedule:work exited` stderr marker flags restarts. Caveat: on Autoscale the container (and scheduler) sleeps with no traffic; ticks happen only while awake. EC2 deployments instead use `deploy/ec2/systemd/sayzio-scheduler.timer` + `sayzio-queue.service`.
- **Admin Integrations Hub**: `/admin/integrations` (`IntegrationsController`) consolidates third-party credentials with status badges; `IntegrationCatalog` assembles statuses. `PlatformServiceSettings` brings Google Places/Trustpilot keys, Google Contacts OAuth, Google Calendar OAuth (Service Booking sync), and S3 user-content storage under admin control (mirrors `MailSettings`: `app_settings` storage, encrypted secrets, admin→config→env fallback, `applyRuntimeConfig()` at boot so readers need no changes).

...
export default function NewInvoiceScreen() {
  ...
  const [vaultClientId, setVaultClientId] = useState<number | null>(null);
  ...
  const companiesQ = useQuery({ queryKey: ["billing-companies"], queryFn: listCompanies });
  ...
  const subtotalMinor = useMemo(
    () =>
      lines.reduce(
        (s, l) => s + toMinor(l.amount) * (Number(l.quantity) || 1),
        0,
      ),
    [lines],
  );
  ...
  const create = useMutation({
    ...
    onSuccess: (inv) => {
      ...
      Alert.alert(
        mode === "receipt" ? "Receipt created" : "Invoice created",
  ...
  const canSubmit = () => {
    ...
    if (!contact && !vaultClientId && !recipientEmail.trim() && !recipientName.trim()) {
      return false;
    }
    return true;
  };
  ...
  const recipientLabel = contact
    ...
    : vaultClientId
      ? clientsQ.data?.find((c) => c.id === vaultClientId)?.name ?? "Client"
      : null;

  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <Stack.Screen options={{ title: mode === "receipt" ? "New receipt" : "New invoice" }} />
      <ScrollView
        contentContainerStyle={{ padding: 20, gap: 16, paddingBottom: 48 }}
        keyboardShouldPersistTaps="handled"
      >
        {/* Mode toggle */}
        <View style={[styles.segment, { borderColor: colors.border, borderRadius: colors.radius }]}>
          {(["invoice", "receipt"] as Mode[]).map((m) => (
            <Pressable
              key={m}
              onPress={() => setMode(m)}
              style={[
        ...
        <View style={cardStyle}>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            <View style={{ flex: 1 }}>
              <Button
                label={contact ? "Contact ✓" : "Pick contact"}
          ...
          {letterhead ? (
            <View style={{ gap: 10 }}>
              <Image
                ...
                resizeMode="cover"
              />
              <Button label="Remove letterhead" variant="ghost" onPress={() => setLetterhead(null)} />
            </View>
          ) : (
              ...
              {(["portrait", "landscape"] as const).map((o) => (
                <Pressable
                  ...
                  style={{
                    ...
                    backgroundColor:
                      orientation === o ? colors.primary + "1c" : colors.background,
                  }}
                >
                  <Text
                    style={{
        ...
        <View style={cardStyle}>
          ...
          <View style={{ gap: 6, marginTop: 4 }}>
            ...
            <View style={[styles.totalRow, { borderTopWidth: StyleSheet.hairlineWidth, borderColor: colors.border, paddingTop: 8 }]}>
              <Text style={[styles.label, { color: colors.foreground }]}>
                {mode === "receipt" ? "Total collected" : "Total"}
              </Text>
        ...
        <Button
          label={mode === "receipt" ? "Create receipt" : "Create invoice"}
          onPress={submit}

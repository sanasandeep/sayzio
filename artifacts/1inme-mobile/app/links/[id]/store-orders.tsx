...
import {
  ...
  ORDER_STATUS_FLOW,
  OPEN_ORDER_STATUSES,
  pollStoreOwnerOrders,
  updateStoreOwnerOrderStatus,
  type OwnerOrder,
...
export default function StoreOrdersScreen() {
  ...
  // Deep-links land on "all" so the highlighted request is visible even when closed.
  ...
  const initial = useQuery({
    queryKey: ["store-owner-orders", linkId],
    queryFn: () => getStoreOwnerOrders(linkId),
  ...
  const setStatus = useMutation({
    mutationFn: ({ orderId, status }: { orderId: number; status: string }) =>
      updateStoreOwnerOrderStatus(linkId, orderId, status),
    onSuccess: (order) => {
      merge([order]);
      qc.invalidateQueries({ queryKey: ["store-owner-orders", linkId] });
    },
  });
        ...
        {visible.length === 0 ? (
          <View style={styles.center}>
            ...
            <Text style={{ color: colors.mutedForeground, marginTop: 12 }}>
              No requests yet. New requests appear here automatically.
            </Text>
          </View>
        ) : null}
          ...
          >
            <View style={styles.cardHead}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.table, { color: colors.foreground }]}>
                  {o.customer_name ? o.customer_name : "Request"} #{o.id}
                </Text>
                {o.customer_contact ? (
            ...
            <View style={[styles.total, { borderTopColor: colors.border }]}>
              <View style={[styles.breakdownRow, { marginTop: 4 }]}>
                ...
                <Text style={{ color: colors.foreground, fontWeight: "700" }}>
                  {o.currency} {Number(o.total ?? o.subtotal).toFixed(2)}
                </Text>
              </View>
              <Text
                style={{
              ...
              >
                Estimated total, not the actual bill. No payment collected here.
              </Text>
            </View>
            ...
            <View style={styles.actions}>
              {(ORDER_STATUS_FLOW[o.status] ?? []).map((s) => (
                <Pressable
                  ...
                  onPress={() => setStatus.mutate({ orderId: o.id, status: s })}
                  style={[
                    styles.actionBtn,
                    s === "cancelled"

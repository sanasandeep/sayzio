# Validation wrapper for the Website+Website split-view Electron harness.
#
...
#   4. Runs tests/e2e-split-view/run.cjs under xvfb.
#   5. ALWAYS restores the Node-ABI better-sqlite3 binary afterwards (trap),
#      so vitest/node consumers keep working.
#
# Usage: bash artifacts/zio-browser/tests/e2e-split-view/run-validation.sh
set -euo pipefail
...
echo "── Running split-view harness under xvfb ──"
xvfb-run -a node tests/e2e-split-view/run.cjs

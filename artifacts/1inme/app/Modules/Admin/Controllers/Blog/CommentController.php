...
class CommentController extends Controller
{
    ...
    private function notifyApproval(BlogComment $c): void
    {
        try {
            ...
            if ($c->author_email) {
                $url = \App\Modules\Common\Support\PlatformHosts::outboundUrl($c->post ? route('site.blogs.show', $c->post->slug) . '#comment-' . $c->id : url('/'));
    ...
    private function notifyOriginalCommenter(BlogComment $original, BlogComment $reply): void
    {
        try {
            ...
            if ($original->author_email) {
                $url = \App\Modules\Common\Support\PlatformHosts::outboundUrl($original->post ? route('site.blogs.show', $original->post->slug) . '#comment-' . $reply->id : url('/'));

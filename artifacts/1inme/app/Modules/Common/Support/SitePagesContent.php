...
class SitePagesContent
{
     ...
     * pages. This generation adds: the configurable governing-law
     * jurisdiction (Hyderabad, Telangana, India by default), explicit
     * disclosure of AWS data hosting (RDS/S3) and the AI sub-processors
     * (OpenAI, Whisper, ElevenLabs) that process user content, the
     * analytics / click-and-scan tracking and optional marketing-pixel
     * disclosures, and {{company_*}} / {{jurisdiction*}} tokens that are
     * resolved from {@see CompanyIdentity} at render time.
     *
     * Each section keeps a stable `id` so the seeder/migration can merge
     * new sections into admin-edited pages without clobbering edits.
     */
    ...
    {
        $out = [];
        foreach ([self::richDefaults(), self::policyDefaultsV1()] as $set) {

    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-14 space-y-10">
            @foreach($grouped as $group => $items)
                <div>
                    <h2 class="text-sm font-bold uppercase tracking-wider text-blue-400 mb-3">{{ $group }}</h2>
                                ...
                                <div class="flex flex-wrap gap-1.5 sm:justify-end">
                                    ...
                                    @else
                                        @foreach($item['unlocked_by'] as $slug)
                                            ...
                                            <span class="text-[11px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full bg-blue-500/15 text-blue-300 border border-blue-500/20">{{ $name }}</span>
                                        @endforeach
                                    @endif
                                    @if(!empty($item['unit']))

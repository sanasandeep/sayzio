...
<section class="pt-20 pb-10">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 text-center">
        <p class="text-xs uppercase tracking-[0.2em] text-violet-400">Category</p>
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-2">
            ...
            @foreach($categories as $c)
                <a href="{{ route('site.blogs.category', $c->slug) }}" class="px-3 py-1.5 rounded-full text-xs font-medium {{ $c->id === $category->id ? 'bg-violet-600 text-white' : 'bg-white/5 hover:bg-white/10 border border-white/10' }}">{{ $c->name }}</a>
            @endforeach
        </div>
    </div>
</section>

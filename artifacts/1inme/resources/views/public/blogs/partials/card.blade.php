<a href="{{ route('site.blogs.show', $post->slug) }}" class="group block glass-panel rounded-2xl hover:!border-blue-500/40 transition overflow-hidden">
    @if($post->cover_image)
        <div class="aspect-[16/9] bg-white/5 overflow-hidden">
            <img src="{{ \App\Support\PublicStorageUrl::resolve($post->cover_image) }}"@imgSize(\App\Support\PublicStorageUrl::resolve($post->cover_image)) alt="" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" loading="lazy">
        </div>
    @else
        ...
        @if($post->author)
            <div class="mt-3 flex items-center gap-2 text-[11px] text-white/60">
                @if(!empty($post->author->avatar))
                    <img src="{{ \App\Support\PublicStorageUrl::resolve($post->author->avatar) }}"@imgSize(\App\Support\PublicStorageUrl::resolve($post->author->avatar)) alt="" class="w-5 h-5 rounded-full object-cover">
                @else

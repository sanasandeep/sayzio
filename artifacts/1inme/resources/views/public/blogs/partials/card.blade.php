<a href="{{ route('site.blogs.show', $post->slug) }}" class="group block bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden hover:border-violet-500/40 transition">
    @if($post->cover_image)
    ...
    <div class="p-5">
        @if($post->category)
            <span class="inline-block text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full" style="background: {{ $post->category->color ? $post->category->color . '22' : 'rgba(124,58,237,.15)' }}; color: {{ $post->category->color ?: '#a78bfa' }};">{{ $post->category->name }}</span>
        @endif
        <h3 class="mt-2 text-lg font-semibold text-white group-hover:text-violet-300 transition line-clamp-2">{{ $post->title }}</h3>
        @if($post->excerpt)
        ...
        @if($post->author)
            <div class="mt-3 flex items-center gap-2 text-[11px] text-white/60">
                ...
                @else
                    <span class="w-5 h-5 rounded-full bg-violet-600/40 inline-flex items-center justify-center text-[9px] font-bold text-white">{{ strtoupper(mb_substr($post->author->name ?? '?', 0, 1)) }}</span>
                @endif

            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($relatedBlog as $p)
                    <a href="{{ route('site.blogs.show', $p->slug) }}" class="block bg-white/[0.04] hover:bg-white/[0.07] border border-white/10 rounded-2xl overflow-hidden transition">
                        @if($p->cover_image)
                            <div class="aspect-[16/9] bg-white/5 overflow-hidden"><img src="{{ \App\Support\PublicStorageUrl::resolve($p->cover_image) }}"@imgSize(\App\Support\PublicStorageUrl::resolve($p->cover_image)) alt="" loading="lazy" class="w-full h-full object-cover"></div>
                        @endif
                        <div class="p-4">

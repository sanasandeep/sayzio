...
<div class="grid gap-4 md:grid-cols-3">
    @foreach(CloudProviderApp::PROVIDERS as $p)
        ...
        <div class="rounded-xl border border-white/10 p-5" style="background: var(--bg-card);">
            ...
            @if(!$configured)
                <p class="text-xs text-gray-400 mb-3">Not configured for this workspace yet. The owner needs to add OAuth credentials.</p>
                <button disabled class="w-full px-3 py-2 rounded-lg bg-white/5 text-gray-500 text-sm cursor-not-allowed">
                    Unavailable
                </button>
            @elseif($conn)
                <div class="text-xs text-gray-400 mb-1">Connected as</div>
                ...
                @if($conn->last_synced_at)
                    <div class="text-[11px] text-gray-500 mb-3">Last synced {{ $conn->last_synced_at->diffForHumans() }}</div>
                @endif
            ...
            @else
                <p class="text-xs text-gray-400 mb-3">Connect your {{ $label }} account to browse and add files.</p>
                <a href="{{ route('user.cloud-oauth.start', $p) }}"

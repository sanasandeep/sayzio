...
<div class="max-w-7xl mx-auto">
    @include('user.partials.page-hero', [
        ...
        'chips' => [
            ['icon' => 'fa-database text-violet-400', 'text' => $accounts->count() . ' connected'],
        ],
    ])
            ...
            <div class="card-premium p-5">
                ...
                @if($accounts->isEmpty())
                    <div class="text-center py-12">
                        <div class="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-4" style="background: linear-gradient(135deg, rgba(236,72,153,0.18), rgba(139,92,246,0.18));">
                            <i class="fas fa-calendar-alt text-2xl text-violet-400"></i>
                        </div>
                        ...
                        <table class="w-full text-sm">
                            ...
                            @foreach($accounts as $a)
                                <tr style="border-top: 1px solid rgba(255,255,255,0.06);">
                                    <td class="py-3 pl-5">
                                        <span class="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider" style="background:rgba(124,58,237,.15);color:#a78bfa">{{ $a->provider }}</span>
                                    </td>
                                    <td class="py-3">
                                    ...
                                    <td class="py-3 pr-5 text-right">
                                        <form method="POST" action="{{ route('user.calendar.sync', $a) }}" class="inline-block">
                                            @csrf
                                            <button class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition" style="background:rgba(255,255,255,.06);color:var(--text-primary);border:1px solid rgba(255,255,255,.08)" onmouseover="this.style.background='rgba(124,58,237,.15)'" onmouseout="this.style.background='rgba(255,255,255,.06)'">

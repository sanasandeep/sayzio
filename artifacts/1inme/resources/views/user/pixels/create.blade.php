    ...
    <form method="POST" action="{{ route('user.pixels.store') }}">
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Facebook Main Pixel" class="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/40" required>
        ...
        <div class="flex items-center justify-end gap-3 mt-4">
            ...
            <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium">Add Pixel</button>
        </div>
    </form>
</div>
@endsection

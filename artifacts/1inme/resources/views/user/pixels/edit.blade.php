...
<div class="max-w-xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Edit Pixel</h1>
    </div>
    ...
    <form method="POST" action="{{ route('user.pixels.update', $pixel) }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $pixel->name) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
            ...
            <div>
                ...
                <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500" required>
                    @foreach(['facebook', 'google_analytics', 'google_tag_manager', 'linkedin', 'twitter', 'pinterest', 'tiktok', 'snapchat', 'custom'] as $t)
            ...
            <div>
                ...
                <input type="text" name="pixel_id" value="{{ old('pixel_id', $pixel->pixel_id) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" required>
            </div>
        </div>

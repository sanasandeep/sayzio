...
<div class="max-w-3xl mx-auto" x-data="{ category: 'artist_creator' }">
    <div class="mb-6">
        <a href="{{ route('user.verification.index') }}" class="text-xs font-medium transition-colors hover:text-purple-400" style="color: var(--text-muted);">
    ...
    <form action="{{ route('user.verification.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="card-premium p-6">
            <h3 class="text-sm font-bold mb-4" style="color: var(--text-primary);"><i class="fas fa-link text-purple-400 mr-2"></i>Select Biolink Page</h3>
        ...
        <div class="card-premium p-6">
            <h3 class="text-sm font-bold mb-4" style="color: var(--text-primary);"><i class="fas fa-tag text-blue-400 mr-2"></i>Category</h3>
            <div class="grid grid-cols-2 gap-3">
                <label class="cursor-pointer p-4 rounded-xl transition-all" :class="category === 'artist_creator' ? 'ring-2 ring-purple-500' : ''" style="background: var(--bg-glass); border: 1px solid var(--border-glass);" @click="category = 'artist_creator'">
                    <input type="radio" name="category" value="artist_creator" x-model="category" class="sr-only">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: rgba(124,58,237,0.1);"><i class="fas fa-palette text-purple-400"></i></div>
                        <div>
                ...
                <label class="cursor-pointer p-4 rounded-xl transition-all" :class="category === 'business_product' ? 'ring-2 ring-blue-500' : ''" style="background: var(--bg-glass); border: 1px solid var(--border-glass);" @click="category = 'business_product'">
                    <input type="radio" name="category" value="business_product" x-model="category" class="sr-only">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: rgba(59,130,246,0.1);"><i class="fas fa-building text-blue-400"></i></div>
                        <div>
        ...
        <div class="card-premium p-6">
            ...
            <label class="cursor-pointer block p-6 rounded-xl text-center transition-all hover:bg-white/[0.03]" style="background: var(--bg-glass); border: 2px dashed var(--border-glass);">
                <i class="fas fa-folder-open text-purple-400 text-2xl mb-2"></i>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-medium text-white transition-all hover:-translate-y-0.5" style="background: linear-gradient(135deg, #7c3aed, #a855f7);">

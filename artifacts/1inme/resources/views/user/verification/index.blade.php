...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <a href="{{ route('user.verification.request') }}" class="px-4 py-2 rounded-xl text-sm font-medium text-white transition-all hover:-translate-y-0.5" style="background: linear-gradient(135deg, #7c3aed, #a855f7);">
        ...
        <div class="card-premium p-5">
            <div class="flex items-center justify-between flex-wrap gap-3">
                <div class="flex items-center gap-4">
                    ...
                    @else
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background: rgba(124,58,237,0.1);"><i class="fas fa-building text-purple-400"></i></div>
                    @endif
                    <div>
                        <div class="flex items-center gap-2 mb-0.5">
                            <span class="text-sm font-bold" style="color: var(--text-primary);">{{ $req->display_name }}</span>
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold {{ $req->category === 'artist_creator' ? 'bg-purple-500/10 text-purple-400' : 'bg-blue-500/10 text-blue-400' }}">
    ...
    <div class="card-premium p-10 text-center">
        ...
        <a href="{{ route('user.verification.request') }}" class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-medium text-white transition-all hover:-translate-y-0.5" style="background: linear-gradient(135deg, #7c3aed, #a855f7);">
            <i class="fas fa-plus mr-1.5"></i>Request Verification
        </a>
    </div>
    @endif

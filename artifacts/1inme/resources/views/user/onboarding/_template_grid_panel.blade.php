...
@if($recommended->isEmpty() && $others->isEmpty())
    <div class="glass rounded-2xl border border-white/10 p-10 text-center">
        ...
        <form method="POST" action="{{ route('user.onboarding.template.apply') }}">
            @csrf
            <button type="submit" name="skip" value="1" class="inline-block px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold">Continue to dashboard</button>
        </form>
    </div>
@else

...
<div class="container mx-auto px-4 py-6 max-w-6xl">
    <div class="flex items-start justify-between gap-4 flex-wrap mb-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Triage public submissions, push them to your kanban, ship and notify upvoters.</p>
        </div>
    ...
    @else
        <div class="flex flex-wrap items-center gap-2 mb-4 text-sm">
            ...
            @foreach($tabs as $key => $label)
                ...
                <a href="{{ route('user.roadmap.triage', ['link' => $link, 'status' => $key, 'block_id' => $blockId]) }}"
                   class="px-3 py-1.5 rounded-full border {{ $status === $key ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-700 border-gray-200 hover:border-purple-300' }}">
            ...
            @if($blocks->count() > 1)
                <form method="GET" class="ml-auto">
                    ...
                    <select name="block_id" onchange="this.form.submit()" class="text-sm rounded-lg border-gray-200">
        ...
        <div class="space-y-3">
            @forelse($items as $item)
                <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                    <div class="flex items-start gap-3">
                        <div class="text-center px-2 py-1 rounded-lg bg-gray-50 min-w-[52px]">
                            <div class="text-xl font-bold text-purple-600 tabular-nums">{{ $item->votes_count }}</div>
                            <div class="text-[10px] uppercase tracking-wider text-gray-500">votes</div>
                        </div>
                        <div class="flex-1 min-w-0">
                            <h3 class="font-semibold text-gray-900">{{ $item->title }}</h3>
                            @if($item->description)
                                <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ \Illuminate\Support\Str::limit($item->description, 400) }}</p>
                            @endif
                    ...
                    <form method="POST" action="{{ route('user.roadmap.update', ['link' => $link, 'item' => $item]) }}" class="mt-3 flex flex-wrap gap-2 items-center border-t pt-3">
                        @csrf @method('PATCH')
                        <select name="status" class="text-sm rounded-lg border-gray-200">
                            @foreach(\App\Modules\User\Models\RoadmapItem::STATUSES as $k => $label)
                                <option value="{{ $k }}" {{ $item->status === $k ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        <label class="text-xs flex items-center gap-1 text-gray-700">
                    ...
                    <div class="mt-2 flex flex-wrap gap-2 items-center">
                        <form method="POST" action="{{ route('user.roadmap.merge', ['link' => $link, 'item' => $item]) }}" class="flex items-center gap-1">
                            @csrf
                            <input type="number" name="into_id" placeholder="Merge into ID" class="text-xs rounded-lg border-gray-200 w-32" min="1">
                            <button type="submit" class="text-xs px-2 py-1 rounded-lg border border-gray-200 hover:bg-gray-50">
            ...
            @empty
                <div class="p-6 bg-gray-50 border border-dashed border-gray-200 rounded-xl text-center text-gray-500 text-sm">

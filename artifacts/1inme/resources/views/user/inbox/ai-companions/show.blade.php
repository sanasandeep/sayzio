...
<div class="max-w-3xl mx-auto px-4 py-8" x-data="cmpInboxChat()">
    ...
    <div class="flex items-center gap-3 mb-4">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center text-violet-300 bg-violet-500/15"><i class="fas fa-robot"></i></div>
        <div>
    ...
    <div id="cmp-thread" class="rounded-2xl border border-white/10 bg-white/[0.03] p-4 space-y-3 min-h-[420px] max-h-[540px] overflow-y-auto">
        @forelse($messages as $m)
            <div class="flex gap-2 {{ $m->role === 'user' ? 'justify-end' : '' }}">
                <div class="max-w-[80%] px-3 py-2 rounded-2xl text-sm whitespace-pre-wrap break-words {{ $m->role === 'user' ? 'bg-violet-500/20 text-white' : 'bg-white/5 text-white/90' }}">
                    {{ $m->content }}
                </div>
            </div>
        @empty
            <p class="text-center text-sm text-white/40 py-12">Say hello — your Companion will reply using its persona.</p>
        @endforelse
    </div>

    <form @submit.prevent="send" class="mt-3 flex gap-2">
        @csrf
        <input type="text" x-model="text" :disabled="sending" placeholder="Message {{ $companion->name }}…" class="flex-1 rounded-xl bg-white/5 border border-white/10 px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-violet-500/50">
        <button type="submit" :disabled="sending || !text.trim()" class="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold disabled:opacity-40">
...
function cmpInboxChat() {
    return {
        ...
        append(role, body) {
            ...
            const cls = role === 'user' ? 'bg-violet-500/20 text-white' :

...
const FLOW = @json([
    'actions' => $flow->actions->map(fn($a) => ['client_id' => 'a' . $a->id, 'kind' => $a->kind, 'label' => $a->label, 'payload' => $a->payload])->values(),
    'steps'   => $flow->steps->map(fn($s) => [
        'key' => $s->key, 'kind' => $s->kind, 'message_text' => $s->message_text,
        'answer_field' => $s->answer_field, 'is_entry' => (bool)$s->is_entry, 'skip_if_known' => (bool)$s->skip_if_known,

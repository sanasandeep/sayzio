...
@section('title', 'Bulk Biolink — Results')
        ...
        <table class="w-full text-sm">
            <thead class="bg-white/5 border-b border-white/10 text-[11px] uppercase tracking-wider text-white/40">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left">Public URL / Issue</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="{{ $r['status'] === 'created' ? '' : 'bg-amber-500/5' }}">
                    ...
                    <td class="px-4 py-3 align-top">
                        <span class="text-xs text-white/70 break-all">{{ $r['title'] }}</span>
                    </td>
                    <td class="px-4 py-3 align-top">
                        @if($r['status'] === 'created')
...
document.addEventListener('alpine:init', () => {
    window.Alpine.data('bulkBiolinkResults', (rows) => ({
        rows,
        downloadCsv() {
            const escape = (v) => {
                const s = (v ?? '').toString();
            ...
            const header = ['title', 'public_url', 'alias', 'status', 'error'];
            ...
            a.href = url;
            a.download = 'bulk-biolinks-' + new Date().toISOString().slice(0, 10) + '.csv';

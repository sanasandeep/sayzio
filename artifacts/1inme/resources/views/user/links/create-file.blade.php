    ...
    <form method="POST" action="{{ route('user.links.file.store') }}" enctype="multipart/form-data">
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $prefillTitle ?? '') }}" placeholder="Optional title (defaults to file name)" class="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/40">
            </div>
        ...
        <div class="flex items-center justify-end gap-3 mt-4">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-sm font-medium">Create File Share</button>
        </div>
    </form>
</div>
@endsection

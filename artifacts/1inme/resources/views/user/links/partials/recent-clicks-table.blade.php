...
<div class="overflow-x-auto">
    <table class="w-full text-xs">
        ...
        <tr class="hover:bg-white/[0.02]" style="border-top: 1px solid var(--border-glass);">
            ...
            <td class="py-2 px-2" style="color: var(--text-muted);">@if($c->block_id)<span class="badge text-[10px]" style="background:rgba(27,132,255,0.08); color:#7fbbff;">{{ ($blockTypes[$c->block_type]['label'] ?? $c->block_type) }}</span>@else<span style="color:var(--text-faint);">page</span>@endif</td>
        </tr>
        @endforeach
        </tbody>
    </table>
...
<div class="mt-4 flex items-center justify-between flex-wrap gap-3">
    ...
    <nav class="flex items-center gap-1.5">
        ...
        @for($i = $start; $i <= $end; $i++)
            @if($i == $current)
                <span class="px-3 py-1.5 rounded-lg text-xs font-bold" style="background: linear-gradient(135deg, #3e97ff, #7fbbff); color: white; border: 1px solid rgba(27,132,255,0.4);">{{ $i }}</span>
            @else

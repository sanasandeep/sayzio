...
<div class="glass rounded-2xl p-6 mb-6" x-data="smartRulesEditor(@js($smartRulesData))">
    <div class="flex items-start gap-3 mb-4 pb-4 border-b border-white/5">
        <div class="w-9 h-9 rounded-xl bg-violet-500/15 text-violet-300 flex items-center justify-center text-base shrink-0">
        ...
        <div class="border border-white/10 rounded-xl p-4 mb-3 bg-white/[0.02]">
            ...
            <div x-show="rule.type === 'country'" class="space-y-3">
                <div>
                    ...
                    <input type="text" :value="rule.match.join(', ')" @input="rule.match = $event.target.value.split(',').map(s => s.trim().toUpperCase()).filter(s => /^[A-Z]{2}$/.test(s))" placeholder="IN, US" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
                </div>
            </div>
            ...
            <div x-show="rule.type === 'language'" class="space-y-3">
                <div>
                    ...
                    <input type="text" :value="rule.match.join(', ')" @input="rule.match = $event.target.value.split(',').map(s => s.trim().toLowerCase()).filter(s => /^[a-z]{2,3}$/.test(s))" placeholder="hi, en" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
                </div>
            </div>
            ...
            <div x-show="rule.type === 'time'" class="space-y-3">
                <div class="grid grid-cols-3 gap-2">
                    <div>
                        ...
                        <input type="time" x-model="rule.from" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
                    </div>
                    <div>
                        <label class="block text-xs text-white/50 mb-1">To</label>
                        <input type="time" x-model="rule.to" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
                    </div>
                    <div>
                        <label class="block text-xs text-white/50 mb-1">Time zone</label>
                        <select x-model="rule.tz" class="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
            ...
            <div x-show="rule.type !== 'ab'" class="mt-3">
                ...
                <input type="url" x-model="rule.url" placeholder="https://example.com/destination" class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
            </div>
            ...
            <div x-show="rule.type === 'ab'" class="space-y-2">
                ...
                <template x-for="(v, j) in rule.variants" :key="j">
                    <div class="flex items-center gap-2">
                        ...
                        <input type="url" x-model="v.url" placeholder="https://variant.example" class="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40">
                        <input type="number" min="1" max="100" x-model.number="v.weight" class="w-20 bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-sm text-white focus:ring-2 focus:ring-violet-500/40" title="Weight">

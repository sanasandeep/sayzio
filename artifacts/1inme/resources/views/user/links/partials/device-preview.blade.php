<style>
    ...
    /* Manual zoom control — lets creators enlarge the previewed page for
       legibility. 100% == the auto-fit baseline (phone fits the column);
       higher values widen the phone frame so the scaled iframe renders the
       page bigger and more readable. */
    .preview-zoom-slider {
    ...
    /* Scroll stage: when the creator zooms in past the fit baseline the phone
       frame grows wider than its column, so allow horizontal scroll (safe
       centering keeps the start reachable) instead of clipping the page. */
...
<div class="device-preview-root">
    ...
    {{-- Single mobile-first preview. Creators view the page the way most
         visitors will (on a phone); the pop-out opens a larger full-screen
         view for a closer look, reusing the same signed/draft-aware URL. --}}
    <div class="flex items-center justify-center gap-2 mb-2">
        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-medium" style="color: var(--text-faint); background: rgba(255,255,255,0.03); border: 1px solid var(--border-glass);">
            <i class="fas fa-mobile-alt text-[9px]"></i> Mobile preview
        </span>
    ...
    {{-- Manual zoom — enlarges the previewed page for a closer read. 100% is
         the auto-fit baseline (the phone fits its column); dragging up widens
         the phone frame so the scaled page renders bigger and more legible.
    ...
    <div class="flex items-center justify-center gap-2 mb-3 text-[10px]"
        ...
        <i class="fas fa-magnifying-glass-minus text-[10px]" style="color: var(--text-faint);" title="Fit"></i>
        ...
        <button type="button" @click="set(100)" x-show="zoom !== 100" x-cloak class="preview-zoom-reset" title="Reset to fit">Fit</button>
    </div>

    <div class="device-preview-stage">
        {{-- Phone — the single mobile-first preview surface. --}}
        <div class="device-frame-phone relative mx-auto">
...
// baseline where the phone fits its column; >1 widens the phone frame so the
// scaled page renders bigger and more legible). Persisted per-tab in
// sessionStorage.
var _previewZoom = 1;
...
// Apply the manual zoom factor by WIDENING the phone frame past its auto-fit
// baseline (rather than shrinking a scaled-down frame). A wider frame means
// the iframe — which is scaled to the frame width by _scaleSingleIframe —
// renders the page bigger and more legible. At 100% the inline width is
// cleared so the responsive CSS fit width takes over again.
function _applyZoom() {
    var frame = document.querySelector('.device-frame-phone');
    if (!frame) return;
    // Reset to the CSS-driven fit width first so we always measure the true
    // baseline, then widen from there when zoomed in.
    frame.style.width = '';
    if (_previewZoom > 1) {
        var base = frame.getBoundingClientRect().width;
        if (base > 0) frame.style.width = Math.round(base * _previewZoom) + 'px';
    }
    _scaleDeviceIframes();
}
...
// Alpine component backing the manual zoom slider. Persists to sessionStorage
// so the zoom survives a tab-local refresh but doesn't leak across tabs. Range
// is 100–200%: 100 is the auto-fit baseline, higher values widen the phone.
    ...
    return {
        ...
        restore() {
            try {
                ...
                if (!isNaN(z) && z >= 100 && z <= 200) this.zoom = z;
        ...
        apply() {
            ...
            if (isNaN(z)) z = 100;
            z = Math.max(100, Math.min(200, Math.round(z / 10) * 10));
            this.zoom = z;
            _previewZoom = z / 100;
            try {
...
// Debounced/coalesced preview reload. Adding a block then persisting its order
// (or several quick edits in a row) used to fire multiple full public-page
// iframe reloads back-to-back — expensive over the remote RDS and visibly
// janky. Collapsing a burst of refresh requests into a single reload keeps the
// preview accurate to the latest edit while reloading the (slow) page once.
var _refreshPreviewTimer = null;
function refreshPreview() {
    if (_refreshPreviewTimer) clearTimeout(_refreshPreviewTimer);
    _refreshPreviewTimer = setTimeout(function() {
        _refreshPreviewTimer = null;
        _doRefreshPreview();
    }, 300);
}
function _doRefreshPreview() {
    ...
    // Pop-out is always visible while open, so keep it live too.
    if (_isPopoutOpen()) _reloadPopoutIframe();
}

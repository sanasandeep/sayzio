...
<div class="mt-4 pt-4" style="border-top: 1px solid var(--border-subtle);" x-data="{ showDisplay: false }">
    <button type="button" @click="showDisplay = !showDisplay"
        ...
        <span><i class="fas fa-sliders-h mr-2 text-purple-400"></i>Display Settings</span>
    ...
    <div x-show="showDisplay" x-cloak x-transition class="mt-3 space-y-4">
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fas fa-globe text-purple-400"></i> Continents
                <span class="text-purple-400/60">({{ count($vis['continents'] ?? []) ?: 'All' }})</span>
            </button>
            <div x-show="open" x-cloak class="mt-2 grid grid-cols-2 gap-1.5">
                ...
                <label class="flex items-center gap-2 text-xs cursor-pointer px-2 py-1.5 rounded-lg" style="color: var(--text-muted); background: var(--bg-glass-input);">
                    <input type="checkbox" name="visibility[continents][]" value="{{ $c }}"
                           ...
                           class="rounded border-white/20 bg-white/5 text-purple-500 focus:ring-purple-500/30">
                    {{ $c }}
                </label>
                @endforeach
            </div>
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fas fa-flag text-purple-400"></i> Countries
                <span class="text-purple-400/60">({{ count($vis['countries'] ?? []) ?: 'All' }})</span>
            </button>
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fas fa-city text-purple-400"></i> Cities
                <span class="text-purple-400/60">({{ count($vis['cities'] ?? []) ?: 'All' }})</span>
            </button>
            <div x-show="open" x-cloak class="mt-2">
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fas fa-desktop text-purple-400"></i> Devices
                <span class="text-purple-400/60">({{ count($vis['devices'] ?? []) ?: 'All' }})</span>
            </button>
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fab fa-windows text-purple-400"></i> Operating Systems
                <span class="text-purple-400/60">({{ count($vis['os'] ?? []) ?: 'All' }})</span>
            </button>
            <div x-show="open" x-cloak class="mt-2 grid grid-cols-2 gap-1.5">
                @foreach($allOs as $o)
                <label class="flex items-center gap-2 text-xs cursor-pointer px-2 py-1.5 rounded-lg" style="color: var(--text-muted); background: var(--bg-glass-input);">
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fab fa-chrome text-purple-400"></i> Browsers
                <span class="text-purple-400/60">({{ count($vis['browsers'] ?? []) ?: 'All' }})</span>
            </button>
            <div x-show="open" x-cloak class="mt-2 grid grid-cols-2 gap-1.5">
        ...
        <div x-data="{ open: false }">
            <button type="button" @click="open = !open" class="text-xs font-medium flex items-center gap-1.5" style="color: var(--text-muted);">
                <i class="fas fa-language text-purple-400"></i> Browser Languages
                <span class="text-purple-400/60">({{ count($vis['languages'] ?? []) ?: 'All' }})</span>
            </button>
            <div x-show="open" x-cloak class="mt-2">

...
@section('title', 'Create Paid Page')
...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Create Paid Page</h1>
    ...
    <form method="POST" action="{{ route('user.links.store') }}">
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <p class="text-sm text-white/50">
                A bold, shareable home for your paid content. It reuses your existing posts, subscription tiers, pay-per-view unlocks and tipping — just dressed in a vibrant template you pick below. Manage posts &amp; tiers from your dashboard; this page brings them together for fans.
            </p>

            <div>
                <label class="block text-sm font-medium text-white/60 mb-1">Page Title</label>
                <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Jane's VIP Lounge" class="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-violet-500/40">
                ...
                @foreach($templates as $id => $t)
                    <button type="button" @click="tpl = '{{ $id }}'"
                        ...
                        <div class="h-20 relative" style="background: {{ $t['hero_bg'] }};">
                            ...
                            <span x-show="tpl === '{{ $id }}'" class="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-violet-500 text-white text-[10px] flex items-center justify-center"><i class="fas fa-check"></i></span>
                        </div>
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="px-5 py-2.5 rounded-xl text-sm font-semibold bg-violet-600 hover:bg-violet-500 text-white">Create Paid Page</button>
        </div>
    </form>
</div>
@endsection

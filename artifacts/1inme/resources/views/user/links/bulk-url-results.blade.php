...
<div class="max-w-5xl mx-auto" x-data="bulkResults({{ \Illuminate\Support\Js::from($results) }})">
    <div class="flex items-center gap-4 mb-6">
        ...
        <a href="{{ route('user.links.url.bulk') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-xs font-medium transition-all">
                    ...
                    <td class="px-4 py-3 align-top">
                        @if($r['status'] === 'created')
                            <div class="flex items-center gap-2" x-data="{ copied: false }">
                                <a href="{{ $r['short_url'] }}" target="_blank" rel="noopener" class="text-xs text-blue-300 hover:underline font-mono break-all">{{ $r['short_url'] }}</a>
                                <button type="button"

...
<div class="rounded-xl border border-white/10 overflow-hidden" style="background: var(--bg-card);">
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-white/5">
            @forelse($items as $a)
                <tr>
                    <td class="px-4 py-2 text-xs text-gray-400">{{ $a->occurred_at?->toDateTimeString() }}</td>
                    ...
                    <td class="px-4 py-2 text-xs text-gray-500">{{ $a->ip }}</td>
                </tr>
            @empty
                <tr><td colspan="5" class="px-4 py-12 text-center text-gray-400">No vault activity yet.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
@if(!$isAdmin)
    <p class="text-xs text-gray-500 mt-3">Showing your own actions only. Workspace owners and admins see entries from every member.</p>
@endif
@endsection

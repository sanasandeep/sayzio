...
<div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
    <div class="flex items-end justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 mt-1">Page-traffic CO₂ estimates and offset history for {{ $workspace->name }}.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div class="rounded-xl bg-white border p-5">
            <div class="text-xs uppercase tracking-wider text-gray-500">Estimated CO₂ (12mo)</div>
            <div class="mt-2 text-3xl font-bold">{{ number_format($totals['grams_co2'] / 1000, 2) }} <span class="text-base text-gray-400">kg</span></div>
        </div>
        <div class="rounded-xl bg-white border p-5">
            <div class="text-xs uppercase tracking-wider text-gray-500">Offset</div>
            <div class="mt-2 text-3xl font-bold text-emerald-700">{{ number_format($totals['grams_offset'] / 1000, 2) }} <span class="text-base text-gray-400">kg</span></div>
        </div>
        <div class="rounded-xl bg-white border p-5">
            <div class="text-xs uppercase tracking-wider text-gray-500">Spend</div>
            <div class="mt-2 text-3xl font-bold">{{ $totals['currency'] }} {{ number_format($totals['cost_minor'] / 100, 2) }}</div>
        </div>
        <div class="rounded-xl bg-white border p-5">
            <div class="text-xs uppercase tracking-wider text-gray-500">Certificates</div>
            <div class="mt-2 text-3xl font-bold">{{ $totals['certificates'] }}</div>
        </div>
    </div>

    <form method="POST" action="{{ route('user.carbon.workspace.update') }}" class="rounded-xl bg-white border p-5 space-y-4">
        @csrf
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold">Workspace defaults</h2>
            <span class="text-xs text-gray-500">Applied to new biolinks unless overridden.</span>
        </div>
        <div class="grid sm:grid-cols-4 gap-4">
            ...
            <label class="text-sm">
                <span class="block text-xs text-gray-500 mb-1">Monthly cap (USD)</span>
                <input type="number" min="0" step="0.01" name="monthly_budget" value="{{ number_format(($workspaceDefaults['monthly_budget_minor'] ?? 0) / 100, 2, '.', '') }}" class="w-full border rounded px-3 py-2">
            </label>
            <label class="text-sm">
                <span class="block text-xs text-gray-500 mb-1">If cap is hit</span>
                <select name="fallback" class="w-full border rounded px-3 py-2">
    ...
    <div class="rounded-xl bg-white border overflow-hidden">
        <div class="px-5 py-3 border-b flex items-center justify-between">
            ...
            <span class="text-xs text-gray-500">Last 12 monthly snapshots</span>
        </div>
        <table class="w-full text-sm">
            ...
            @forelse ($perLink as $row)
                <tr>
                    <td class="px-5 py-3">
                        ...
                        <div class="text-xs text-gray-500">/{{ optional($row['link'])->alias }}</div>
                    </td>
            ...
            @empty
                <tr><td colspan="5" class="px-5 py-8 text-center text-gray-500">No snapshots yet — they're written monthly. Run <code>php artisan carbon:snapshot-monthly</code> to generate now.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
            ...
            @forelse ($purchases as $p)
                <tr>
                    ...
                    <td class="px-5 py-3">
                        ...
                        @else
                            <span class="text-gray-400 text-xs">{{ $p->status === 'sandbox' ? 'Sandbox' : 'Pending' }}</span>
                        @endif
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="px-5 py-8 text-center text-gray-500">No offset purchases yet.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>

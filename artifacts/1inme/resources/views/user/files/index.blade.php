...
<div x-data="fileManager()" x-init="init()">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <button @click="$refs.fileInput.click()" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 transition-all">
            ...
            <div class="flex-1 w-full sm:w-auto">
                ...
                <div class="w-full rounded-full h-2 overflow-hidden" style="background: var(--bg-glass-input);">
                    <div class="h-full rounded-full transition-all duration-500"
                         :class="quota.percent > 90 ? 'bg-red-500' : quota.percent > 70 ? 'bg-yellow-500' : 'bg-purple-500'"
    ...
    <div class="flex items-center gap-2 mb-4 overflow-x-auto pb-1">
        <template x-for="t in ['all','image','video','audio','document']" :key="t">
            <button @click="activeType = t; loadFiles(1)" class="text-xs px-3 py-1.5 rounded-lg font-medium transition-all whitespace-nowrap"
                    :class="activeType === t ? 'bg-purple-500/20 text-purple-300 ring-1 ring-purple-500/30' : 'hover:text-white/50'"
    ...
    <div x-show="uploading" x-transition class="glass rounded-2xl p-4 mb-4">
        <div class="flex items-center gap-3 mb-2">
            <div class="animate-spin w-5 h-5 border-2 border-purple-400 border-t-transparent rounded-full"></div>
        ...
        <div class="w-full rounded-full h-1.5 overflow-hidden" style="background: var(--bg-glass-input);">
            <div class="h-full bg-purple-500 rounded-full transition-all duration-300" :style="'width: ' + uploadProgress + '%'"></div>
        </div>
    </div>
    ...
    <div x-show="!loading && filteredFiles.length === 0" x-cloak class="glass rounded-2xl p-12 text-center">
        ...
        <button @click="$refs.fileInput.click()" class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-xl text-sm font-medium">
    ...
    <div x-show="filteredFiles.length > 0" x-cloak>
        <div x-show="viewMode === 'grid'" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            <template x-for="file in filteredFiles" :key="file.id">
                <div class="glass rounded-xl overflow-hidden group relative cursor-pointer hover:ring-1 hover:ring-purple-500/30 transition-all"
                    ...
                    <div class="aspect-square relative overflow-hidden" style="background: var(--bg-glass-input);">
                        ...
                        <template x-if="file.type === 'video'">
                            <div class="w-full h-full flex items-center justify-center">
                                ...
                                <i class="fas fa-play-circle text-4xl text-purple-400/60"></i>
                            </div>
                        </template>
                        ...
                        <tr class="border-b hover:bg-white/[0.02] transition-colors cursor-pointer" style="border-color: var(--border-glass);"
                            ...
                            <td class="py-3 px-4 hidden sm:table-cell">
                                <span class="text-[10px] px-2 py-0.5 rounded-full font-medium"
                                      :class="file.type === 'image' ? 'bg-purple-500/10 text-purple-300' : file.type === 'video' ? 'bg-blue-500/10 text-blue-300' : file.type === 'audio' ? 'bg-green-500/10 text-green-300' : 'bg-orange-500/10 text-orange-300'"
        ...
        <div class="glass rounded-2xl w-full max-w-lg overflow-hidden" @click.stop>
            ...
            <div class="p-4">
                ...
                <div class="mt-4 p-2.5 rounded-xl flex items-center gap-2" style="background: var(--bg-glass-input); border: 1px solid var(--border-glass);">
                    ...
                    <button @click="copyUrl(selectedFile)" class="text-[10px] px-2 py-1 rounded-lg bg-purple-500/20 text-purple-300 hover:bg-purple-500/30 transition-colors flex-shrink-0">
                        <i class="fas fa-copy mr-1"></i>Copy
                    </button>
                </div>

                <div class="flex gap-2 mt-4">
                    <a :href="selectedFile?.url" target="_blank" class="flex-1 text-center py-2 rounded-xl text-xs font-medium transition-all hover:bg-purple-500/30" style="background: var(--bg-glass); color: var(--text-secondary); border: 1px solid var(--border-glass);">
    ...
    <div x-ref="dropOverlay"
        ...
        <div x-show="dragging" class="absolute inset-8 border-2 border-dashed border-purple-500/40 rounded-3xl flex items-center justify-center">
            <div class="text-center">
                <i class="fas fa-cloud-upload-alt text-4xl text-purple-400/60 mb-2"></i>

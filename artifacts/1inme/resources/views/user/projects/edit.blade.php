    ...
    <form method="POST" action="{{ route('user.projects.update', $project) }}">
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $project->name) }}" class="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40" required>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500/40">{{ old('description', $project->description) }}</textarea>
            </div>
        </div>

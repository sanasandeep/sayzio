...
<div class="max-w-6xl mx-auto space-y-6"
    ...
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold text-white ak-strong">{{ $digest->exists ? 'Edit digest' : 'New digest' }}</h2>
        <div class="flex items-center gap-3">
            @if($digest->exists)
                <a href="{{ route('admin.zio-digests.preview', $digest) }}" target="_blank" class="text-xs text-white/60 hover:text-white ak-muted"><i class="fas fa-eye mr-1"></i> Page preview</a>
                <a href="{{ route('admin.zio-digests.email-preview', $digest) }}" target="_blank" class="text-xs text-white/60 hover:text-white ak-muted"><i class="fas fa-envelope-open-text mr-1"></i> Email preview</a>
            @endif
            <a href="{{ route('admin.zio-digests.index') }}" class="text-xs text-white/60 hover:text-white ak-muted"><i class="fas fa-arrow-left mr-1"></i> Back</a>
        </div>
    </div>
    ...
    @endif
    @if($errors->any())
        <div class="px-3 py-2 bg-red-500/10 border border-red-400/30 text-red-200 rounded-lg text-sm ak-red">{{ $errors->first() }}</div>
    @endif
    ...
    <form method="POST"
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="block text-xs uppercase tracking-wider text-white/40 mb-1 ak-note">Title</label>
                ...
                <div>
                    <label class="block text-xs uppercase tracking-wider text-white/40 mb-1 ak-note">Status</label>
                    <select name="status" class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
            ...
            <div>
                <label class="block text-xs uppercase tracking-wider text-white/40 mb-1 ak-note">Summary <span class="normal-case text-white/30 ak-note">(shown on the page, in emails and WhatsApp)</span></label>
                <textarea name="summary" rows="2" maxlength="5000"
                          class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">{{ old('summary', $digest->summary) }}</textarea>
            </div>
            <div>
                <label class="block text-xs uppercase tracking-wider text-white/40 mb-1 ak-note">Lead image URL</label>
                <div class="flex gap-2">
                    <input type="url" name="lead_image" maxlength="2048" value="{{ old('lead_image', $digest->lead_image) }}" x-ref="leadImage"
                           class="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                    <label class="px-3 py-2 bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg text-xs text-white cursor-pointer ak-strong">
            ...
            @if($digest->exists)
                <p class="text-xs text-white/40 ak-note">Public page: <span class="text-white/70 ak-strong">{{ $digest->publicUrl() }}</span> ({{ $digest->isPublished() ? 'live' : 'hidden while draft' }})</p>
            @endif
        </div>
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div class="flex items-center justify-between">
                <h3 class="text-sm font-semibold text-white ak-strong">Content blocks</h3>
                <div class="flex flex-wrap gap-2 text-xs">
                    <template x-for="t in ['heading','text','image','video','link','embed']" :key="t">
                        <button type="button" @click="addBlock(t)"
                                class="px-2.5 py-1.5 bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg text-white capitalize ak-strong">
                            <i class="fas fa-plus mr-1 text-white/40 ak-note"></i><span x-text="t"></span>
                        </button>
                    </template>
                </div>
            </div>

            <p x-show="blocks.length === 0" class="text-sm text-white/40 py-6 text-center ak-note">No blocks yet — add a heading or text block to get started.</p>

            <template x-for="(block, i) in blocks" :key="block._key">
                <div class="border border-white/10 rounded-xl p-4 space-y-3 bg-white/[0.03]">
                    <div class="flex items-center justify-between">
                        <span class="text-xs uppercase tracking-wider text-white/40 ak-note" x-text="block.type"></span>
                        <div class="flex items-center gap-2 text-white/50 text-xs ak-muted">
                            ...
                            <button type="button" @click="blocks.splice(i, 1)" class="text-red-300/70 hover:text-red-300 ak-red"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                    ...
                    <template x-if="block.type === 'image'">
                        <div class="space-y-2">
                            ...
                            <div class="grid gap-2 md:grid-cols-2">
                                <input type="text" x-model="block.alt" placeholder="Alt text" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                                <input type="text" x-model="block.caption" placeholder="Caption (optional)" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                            </div>
                        </div>
                    </template>
                    ...
                    <template x-if="block.type === 'link'">
                        <div class="grid gap-2 md:grid-cols-2">
                            <input type="url" x-model="block.url" placeholder="https://…" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white md:col-span-2 ak-strong ak-input">
                            <input type="text" x-model="block.title" placeholder="Link title" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                            <input type="text" x-model="block.description" placeholder="Short description (optional)" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                        </div>
                    </template>
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div class="flex items-center justify-between">
                <h3 class="text-sm font-semibold text-white ak-strong">Audience</h3>
                <div class="text-xs text-white/60 ak-muted" x-show="counts">
                    <span class="text-white ak-strong" x-text="counts?.total ?? 0"></span> users —
                    email-eligible <span class="text-emerald-300 ak-green" x-text="counts?.email ?? 0"></span>,
                    WhatsApp-eligible <span class="text-emerald-300 ak-green" x-text="counts?.whatsapp ?? 0"></span>
                    <span class="text-white/40 ak-note">(opted out: <span x-text="counts?.email_opted_out ?? 0"></span>, no phone: <span x-text="counts?.no_phone ?? 0"></span>)</span>
                </div>
            </div>
            <div class="grid gap-3 md:grid-cols-3 text-sm text-white/80 ak-strong">
            ...
            <p class="text-xs text-white/40 ak-note">Email always skips users who unsubscribed from digests; WhatsApp skips users without a phone number. Suspended accounts are never included.</p>
        </div>

        <div class="flex items-center gap-3">
            <button class="px-4 py-2 bg-white/15 hover:bg-white/25 border border-white/10 rounded-lg text-sm text-white ak-strong">
    ...
    @if($digest->exists)
        <div class="grid gap-4 md:grid-cols-2">
            <div class="glass rounded-2xl p-6 space-y-3">
                <h3 class="text-sm font-semibold text-white ak-strong">Send test email</h3>
                <form method="POST" action="{{ route('admin.zio-digests.send-test', $digest) }}" class="flex gap-2">
                    @csrf
                    <input type="email" name="email" required placeholder="you@example.com"
                           class="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                    <button class="px-3 py-2 bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg text-sm text-white ak-strong">Send test</button>
                </form>
                <p class="text-xs text-white/40 ak-note">Sends the current saved version via SendGrid. Save first if you just edited.</p>
            </div>
            <div class="glass rounded-2xl p-6 space-y-3">
                <h3 class="text-sm font-semibold text-white ak-strong">Broadcast</h3>
                <form method="POST" action="{{ route('admin.zio-digests.send', $digest) }}" class="space-y-3"
                    ...
                    <button class="px-3 py-2 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-400/30 rounded-lg text-sm text-emerald-200 ak-green">
                        <i class="fas fa-paper-plane mr-1"></i> Send digest
                    </button>
                    @unless($digest->isPublished())
                        <p class="text-xs text-amber-300/80 ak-amber">Publish the digest before sending — messages link to its public page.</p>
                    @endunless
                </form>
                <a href="{{ route('admin.zio-digests.report', $digest) }}" class="inline-block text-xs text-white/60 hover:text-white ak-muted"><i class="fas fa-chart-simple mr-1"></i> View delivery report</a>
            </div>
        </div>
    @endif
</div>

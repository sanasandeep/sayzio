...
<div class="max-w-5xl space-y-6" x-data="emailTemplateEditor()">
    ...
    <a href="{{ route('admin.email-templates.index') }}" class="inline-flex items-center gap-2 text-xs text-white/50 hover:text-white">
    ...
    <div>
        <div class="flex items-center gap-2">
            <h1 class="text-lg font-semibold text-white">{{ $entry['label'] ?? $key }}</h1>
            @if ($override)
            ...
            @else
                <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/50">Default</span>
            @endif
        </div>
        <p class="text-xs text-white/40 mt-1">{{ $entry['description'] ?? '' }}</p>
        <div class="text-[11px] text-white/30 mt-1">{{ $category }} &middot; <code>{{ $key }}</code></div>
    </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <form method="POST" action="{{ route('admin.email-templates.update', $key) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="subject" x-model="subject"
                       class="w-full rounded-xl bg-white/5 border border-white/10 px-3 py-2 text-sm text-white focus:outline-none focus:border-white/30"
                       value="{{ old('subject', $override['subject'] ?? ($entry['subject'] ?? '')) }}">
            </div>

            <div>
                <label class="block text-xs font-semibold text-white/70 mb-1">Format</label>
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="px-4 py-2 rounded-xl bg-emerald-500/15 border border-emerald-500/25 text-emerald-200 text-xs font-semibold hover:bg-emerald-500/25">
                    Save override
                </button>
                <button type="button" @click="refreshPreview()" class="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white/70 text-xs font-semibold hover:bg-white/10">
                    Update preview
                </button>
            </div>
        </form>
        ...
        <div class="space-y-4">
            <div class="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                <h3 class="text-xs font-semibold text-white/70 mb-2">Available variables</h3>
                @if (!empty($entry['variables']))
                    <table class="w-full text-xs">
                        <tbody class="divide-y divide-white/5">
                        @foreach ($entry['variables'] as $token => $meta)
                            <tr>
                                <td class="py-1 pr-3"><code class="text-white/80">&#123;&#123;{{ $token }}&#125;&#125;</code></td>
                                <td class="py-1 text-white/40">{{ $meta['label'] ?? '' }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
                @else
                    <p class="text-xs text-white/40">This template has no substitutable variables.</p>
                @endif
            </div>

            <div class="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                <h3 class="text-xs font-semibold text-white/70 mb-2">Live preview (sample data)</h3>
                <div class="text-xs text-white/40 mb-1">Subject</div>
                <div class="text-sm text-white mb-3 break-words" x-text="previewSubject"></div>
                <div class="text-xs text-white/40 mb-1">Body</div>
                <template x-if="previewFormat === 'text'">
                    <pre class="text-xs text-white/80 whitespace-pre-wrap bg-black/20 rounded-lg p-3 max-h-96 overflow-auto" x-text="previewBody"></pre>
                </template>
                <template x-if="previewFormat !== 'text'">
            ...
            @if ($override)
                <form method="POST" action="{{ route('admin.email-templates.reset', $key) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-xs font-semibold hover:bg-red-500/20">
                        Reset to default
                    </button>
                </form>
            @endif

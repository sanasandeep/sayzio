    ...
    <div class="flex items-start gap-4">
        ...
        <div class="min-w-0 flex-1">
            ...
            <p class="text-sm text-white/70 mt-1">
                ...
                opened; an all-clear follows the next successful run. Use <span class="text-red-200 font-medium">Run now</span>
                on the job below to retry it immediately.
            </p>
            <ul class="mt-3 space-y-2">
                @if($schedulerEpisode !== null)
                    <li class="text-sm">
                        <span class="font-mono font-semibold text-red-200">scheduler heartbeat</span>
                        <span class="text-white/50">&mdash; the scheduler itself appears to be down (no jobs are firing at all)</span>
                        @if($fmtSince($schedulerEpisode['since'] ?? null))
                            <span class="text-white/40 text-xs block sm:inline sm:ml-1">alerted {{ $fmtSince($schedulerEpisode['since']) }}</span>
                        @endif
                    </li>
                @endif
                @foreach($failingJobs as $episode)
                    <li class="text-sm">
                        <span class="font-mono font-semibold text-red-200">{{ $episode['key'] }}</span>
                        @if($fmtSince($episode['since'] ?? null))
                            <span class="text-white/40 text-xs sm:ml-1">failing since {{ $fmtSince($episode['since']) }}</span>
                        @endif
                        @if(!empty($episode['last_error']))
                            <span class="block text-xs text-white/50 font-mono mt-0.5 break-all">{{ \Illuminate\Support\Str::limit($episode['last_error'], 200) }}</span>
                        @endif
                    </li>
                @endforeach
            </ul>
            @if(!request()->routeIs('admin.cron-jobs.*'))
                <a href="{{ route('admin.cron-jobs.index') }}"
                   class="mt-3 inline-flex items-center gap-2 text-xs text-red-300/80 hover:text-red-200 underline">

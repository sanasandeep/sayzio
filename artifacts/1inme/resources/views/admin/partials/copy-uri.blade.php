...
<div class="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-xs">
    <div class="text-[10px] uppercase tracking-wider text-white/40 mb-1.5">{{ $label ?? 'URL' }}</div>
    <div class="flex items-center gap-2">
        <code class="font-mono text-[12px] text-white/80 break-all flex-1">{{ $value }}</code>
        <button type="button"
                data-copy-val="{{ $value }}"
                ...
                class="shrink-0 text-[11px] px-2 py-1 rounded-md bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 whitespace-nowrap transition">
            Copy
        </button>
    </div>
</div>

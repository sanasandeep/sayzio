...
<div class="max-w-5xl" x-data="designFix()">
    <a href="{{ route('admin.templates.index', ['tab' => $kind]) }}" class="text-xs text-white/40 hover:text-white mb-4 inline-block ak-note">
    ...
    <div class="glass rounded-2xl border border-red-500/30 bg-red-500/5 p-4 mb-5">
        <div class="flex items-start gap-3">
            <i class="fas fa-bug text-red-400 text-lg mt-0.5 ak-red"></i>
            <div class="flex-1">
    ...
    <div class="glass rounded-2xl border border-white/10 p-5 mb-5">
        <h3 class="text-sm font-semibold text-white mb-3 ak-strong">Concrete issues found</h3>
        <ul class="space-y-2">
            @foreach($issues as $issue)
                <li class="flex items-start gap-2 text-xs text-white/70 ak-strong">
                    <i class="fas fa-circle-exclamation text-red-400 mt-0.5 text-[11px] ak-red"></i>
                    <span>{{ $issue }}</span>
                </li>
            @endforeach
        </ul>
    ...
    @if($errors->any())
        <div class="glass rounded-xl border border-red-500/30 bg-red-500/5 p-3 mb-5 text-xs text-red-300 ak-red">
            <ul class="space-y-1">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        <div class="glass rounded-2xl border border-white/10 p-5 flex flex-col">
            <h3 class="text-sm font-semibold text-white mb-1 ak-strong">
                <i class="fas fa-eraser text-blue-300 mr-1 ak-blue"></i>Strip stale design-variant keys
            </h3>
            <p class="text-xs text-white/50 mb-3 flex-1 ak-muted">
                ...
                kept exactly as-is.
                @if($stripFullyFixes)
                    <span class="block mt-2 text-emerald-300 ak-green"><i class="fas fa-check mr-1"></i>This alone fully resolves every issue on this template.</span>
                @else
                    <span class="block mt-2 text-amber-300 ak-amber"><i class="fas fa-triangle-exclamation mr-1"></i>This won't fully fix the row, other issues (e.g. unknown block types) would remain. Use re-capture instead.</span>
                @endif
            </p>
        ...
        <div class="glass rounded-2xl border border-white/10 p-5">
            <h3 class="text-sm font-semibold text-white mb-1 ak-strong">
                <i class="fas fa-camera text-blue-300 mr-1 ak-blue"></i>Re-capture from a source
            </h3>
            <p class="text-xs text-white/50 mb-3 ak-muted">
                @if($kind === 'card')
            ...
            <form action="{{ route('admin.templates.design.repair', ['kind' => $kind, 'id' => $tpl->id]) }}" method="POST">
                ...
                @if($kind === 'card')
                    ...
                    <p x-show="sourceLinkId && !cards.length" x-cloak class="text-xs text-amber-400 mb-3 ak-amber">No card blocks found in that link.</p>
                @endif
    ...
    <div class="glass rounded-2xl border border-white/10 p-4">
        <div class="flex items-center justify-between mb-2">
            <div class="text-[10px] uppercase tracking-wide text-white/40 ak-note">Current stored snapshot JSON</div>
            <a href="{{ route('admin.templates.edit', ['kind' => $kind, 'id' => $tpl->id]) }}" class="text-xs text-white/40 hover:text-white ak-note">
                <i class="fas fa-edit mr-1"></i>Edit manually instead
            </a>
        </div>
        <pre class="text-[11px] text-white/70 bg-black/40 rounded-lg p-3 overflow-auto max-h-[50vh] whitespace-pre-wrap break-words ak-strong">{{ $snapshotJson }}</pre>
    </div>
</div>

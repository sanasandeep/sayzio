        ...
        <form method="POST" action="{{ route('admin.roles.store') }}">
            ...
            <div class="space-y-5">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                            ...
                            <label class="flex items-center gap-2 text-sm text-white/60 p-2 rounded hover:bg-white/5">
                                <input type="checkbox" name="permissions[]" value="{{ $perm->id }}" class="rounded border-white/10 text-violet-400 focus:ring-violet-500/40">
                                {{ $perm->name }}
                            </label>
                            @endforeach
                        </div>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <button type="submit" class="px-6 py-2.5 bg-violet-600 text-white rounded-xl font-medium hover:bg-violet-700 transition">Create Role</button>

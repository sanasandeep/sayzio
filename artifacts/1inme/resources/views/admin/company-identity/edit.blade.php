...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="glass rounded-2xl p-6">
        <h2 class="ci-heading text-lg font-semibold text-white mb-1 ak-strong">Legal company identity</h2>
        <p class="ci-desc text-sm text-white/50 mb-4 ak-muted">
        ...
        <div class="ci-info-box rounded-xl px-4 py-3 mb-6 text-sm bg-blue-500/[0.08] border border-blue-500/20 text-blue-300 ak-blue">
        ...
        <form method="POST" action="{{ route('admin.company-identity.update') }}" class="space-y-5">
            ...
            @foreach($fields as $key => $meta)
                <div>
                    <label for="{{ $key }}" class="ci-label block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5 ak-muted">
                        {{ $meta['label'] }}
                    </label>
                    @if(($meta['type'] ?? 'text') === 'textarea')
                        <textarea id="{{ $key }}"
                                  ...
                                  class="ci-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">{{ old($key, $values[$key]) }}</textarea>
                    @else
                        <input type="{{ ($meta['type'] ?? 'text') === 'email' ? 'email' : (($meta['type'] ?? 'text') === 'url' ? 'url' : 'text') }}"
                               ...
                               placeholder="{{ $defaults[$key] ?? '' }}"
                               class="ci-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                    @endif
                    @if(!empty($meta['help']))
                        <p class="ci-help mt-1 text-xs text-white/40 ak-note">{{ $meta['help'] }}</p>
                    @endif
                    @if(empty($values[$key]) && !empty($resolved[$key]))
                        <p class="ci-fallback mt-1 text-xs text-white/40 ak-note">Currently using fallback: <span class="ci-fallback-val text-blue-300 ak-blue">{{ $resolved[$key] }}</span></p>
                    @endif
                    @error($key)

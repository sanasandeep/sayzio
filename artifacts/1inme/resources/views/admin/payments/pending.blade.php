            ...
            <tbody class="text-white/80">
                @forelse($invoices as $inv)
                    <tr class="border-t border-white/5 align-top">
                        ...
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('admin.payments.mark-paid', $inv) }}" class="flex items-center gap-2" onsubmit="return confirm('Mark invoice {{ $inv->number }} as paid and activate the subscription?');">
                                @csrf

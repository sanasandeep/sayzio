...
<div class="space-y-6">
            ...
            <tbody class="text-white/80 ak-strong">
                @forelse($invoices as $inv)
                    <tr class="border-t border-white/5 align-top">
                        <td class="px-4 py-3">
                            <div class="font-mono text-white ak-strong">{{ $inv->number }}</div>
                            <div class="text-xs text-white/50 ak-muted">{{ $inv->created_at?->diffForHumans() }}</div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="text-white ak-strong">{{ $inv->user?->name }}</div>
                            <div class="text-xs text-white/50 ak-muted">{{ $inv->user?->email }}</div>
                        </td>
                        <td class="px-4 py-3 text-right font-mono">{{ number_format($inv->grand_total_minor/100, 2) }} {{ $inv->currency }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs bg-amber-500/20 text-amber-200 ak-amber">{{ $inv->status }}</span>
                        </td>
                        <td class="px-4 py-3">
                            @php($buyerRef = $buyerRefs[$inv->id] ?? null)
                            @if($buyerRef)
                                <div class="mb-1 text-xs text-emerald-200/90 ak-green" title="Buyer-submitted reference, verify against your bank/UPI statement">
                            ...
                            <form method="POST" action="{{ route('admin.payments.mark-paid', $inv) }}" class="flex items-center gap-2" onsubmit="return window.themedConfirmSubmit(this, {title: 'Mark invoice {{ $inv->number }} as paid?', message: 'The subscription will be activated immediately.', confirmText: 'Mark paid', confirmIcon: 'fa-check', iconClass: 'fa-check'})">
                                @csrf
                                <input type="text" name="reference" value="{{ $buyerRef }}" placeholder="Reference #" class="px-2 py-1 rounded bg-white/5 border border-white/10 text-white text-xs w-32 ak-strong ak-input">
                ...
                @empty
                    <tr><td colspan="5" class="px-4 py-8 text-center text-white/40 ak-note">Nothing to review, all caught up.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if($reviewAttempts->count())
        <div class="rounded-2xl border border-amber-400/20 bg-amber-500/5 p-5">
            <h2 class="text-white font-medium mb-2 ak-strong">Flagged gateway attempts</h2>
            <p class="text-xs text-white/50 mb-3 ak-muted">Payments that returned `requires_review` from a real gateway. Check the gateway dashboard before marking the invoice paid.</p>
            <table class="min-w-full text-xs">
                <thead class="text-white/50 ak-muted"><tr><th class="px-2 py-1 text-left">Gateway</th><th class="px-2 py-1 text-left">Ref</th><th class="px-2 py-1 text-left">Invoice</th><th class="px-2 py-1 text-left">When</th></tr></thead>
                <tbody class="text-white/80 ak-strong">

...
<div class="glass rounded-2xl border border-white/10 p-6">
    <div class="flex items-center justify-between mb-4 gap-2 flex-wrap">
        <div>
            <h3 class="text-white font-semibold ak-strong">User-management activity</h3>
            <p class="text-xs text-white/40 ak-note">Plan assignments, coin grants/deductions, account creation, suspensions and reactivations.</p>
        </div>
    ...
    <form method="GET" action="{{ route('admin.users.activity-log.index') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            <label class="block">
                <span class="block text-[10px] uppercase tracking-wide text-white/40 mb-1 ak-note">Operator</span>
            ...
            <label class="block">
                <span class="block text-[10px] uppercase tracking-wide text-white/40 mb-1 ak-note">Target user</span>
        ...
        <table class="w-full text-sm">
            ...
            @foreach($audits as $a)
                <tr class="border-t border-white/5 align-top">
                    <td class="py-2 text-white/60 whitespace-nowrap ak-muted" title="{{ $a->created_at?->toDateTimeString() }}">
                        {{ $a->created_at?->diffForHumans() }}
                    </td>
                    <td class="text-white/80 ak-strong">
                        {{ $a->admin_name ?: 'System' }}
                        @if($a->admin_email)<span class="block text-xs text-white/30 ak-note">{{ $a->admin_email }}</span>@endif
                    </td>
                    <td>
                        <span class="px-2 py-0.5 rounded-md text-xs
                            @switch($a->action)
                                @case('coins.granted') bg-emerald-500/10 text-emerald-300 ak-green @break
                                @case('coins.deducted') bg-rose-500/10 text-rose-300 ak-red @break
                                @case('account.suspended') bg-rose-500/10 text-rose-300 ak-red @break
                                @case('account.reactivated') bg-emerald-500/10 text-emerald-300 ak-green @break
                                @case('account.created') bg-sky-500/10 text-sky-300 ak-blue @break
                                @default bg-blue-500/10 text-blue-300 ak-blue
                            @endswitch">
                            {{ $a->actionLabel() }}
                        </span>
                    </td>
                    <td class="text-white/80 ak-strong">
                        @if($a->target_user_id)
                            ...
                            @if($a->target_email)<span class="block text-xs text-white/30 ak-note">{{ $a->target_email }}</span>@endif
                        @else
                            <span class="text-white/30 ak-note">-</span>
                        @endif
                    </td>
                    <td class="text-white/50 text-xs max-w-xs ak-muted">
                        @if(!empty($a->details))
 ...
 -
                    ...
                    <td class="text-white/30 text-xs whitespace-nowrap ak-note">{{ $a->ip ?: '—' }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>

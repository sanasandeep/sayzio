...
<div class="max-w-2xl mx-auto">
    ...
    <div class="rounded-2xl p-5 mb-5" style="border:1px solid var(--border-glass); background: var(--bg-card);">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="px-2.5 py-1 rounded-full text-[11px] font-bold text-white ak-strong" style="background: {{ $c }};">{{ ucfirst($badgeRequest->status) }}</span>
        </div>
            ...
            <div>
                ...
                <dd style="color: var(--text-primary);">
                    @if($badgeRequest->account_badge_id && $badgeRequest->badge)
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold text-white ak-strong" style="background: {{ $badgeRequest->badge->color }};"><i class="fas fa-certificate text-[10px]"></i> {{ $badgeRequest->badge->name }}</span>
                    @else
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        <form method="POST" action="{{ route('admin.badge-requests.approve', $badgeRequest->id) }}" class="rounded-2xl p-5" style="border:1px solid rgba(16,185,129,0.25); background: rgba(16,185,129,0.04);" x-data="{ src: 'existing' }">
            ...
            <button type="submit" class="w-full py-2.5 rounded-lg text-sm font-bold text-white ak-strong" style="background:#10b981;">Approve</button>
        </form>
        ...
        <form method="POST" action="{{ route('admin.badge-requests.reject', $badgeRequest->id) }}" class="rounded-2xl p-5" style="border:1px solid rgba(239,68,68,0.25); background: rgba(239,68,68,0.04);">
            ...
            <button type="submit" class="w-full py-2.5 rounded-lg text-sm font-bold text-white ak-strong" style="background:#ef4444;">Reject</button>
        </form>
    </div>
    @else

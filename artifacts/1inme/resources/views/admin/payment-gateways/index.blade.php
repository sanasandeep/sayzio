...
<div class="space-y-4">
    ...
    <div class="rounded-2xl border border-white/10 bg-white/[0.02] overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="ak-muted bg-white/5 text-white/60 text-xs uppercase tracking-wider">
                <tr>
            ...
            <tbody class="ak-strong text-white/80">
                @foreach($rows as $r)
                    <tr class="border-t border-white/5">
                        <td class="px-4 py-3">
                            <div class="ak-strong text-white">{{ $r['display_name'] }}</div>
                            <div class="ak-muted text-xs text-white/50 font-mono">{{ $r['slug'] }}</div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <form method="POST" action="{{ route('admin.payment-gateways.toggle', $r['slug']) }}" class="inline">
                                @csrf
                                <button type="submit" class="px-3 py-1 rounded-full text-xs font-medium {{ $r['settings']->is_enabled ? 'ak-tone-green bg-emerald-500/20 text-emerald-200' : 'ak-tone-neutral bg-white/10 text-white/50' }}">
                                    {{ $r['settings']->is_enabled ? 'On' : 'Off' }}
                                </button>
                            </form>
                        </td>
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.payment-gateways.edit', $r['slug']) }}" class="ak-blue text-blue-300 hover:text-blue-200">Configure</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>

...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div class="glass rounded-xl p-4 border border-white/10">
            <div class="ak-note text-[11px] uppercase tracking-wider text-white/40 flex items-center gap-2"><i class="fas fa-plug"></i> Total providers</div>
            <div class="ak-strong text-2xl font-bold text-white mt-1">{{ count($providers) }}</div>
        </div>
        <div class="glass rounded-xl p-4 border border-emerald-500/20">
            <div class="ak-green text-[11px] uppercase tracking-wider text-emerald-300/70 flex items-center gap-2"><i class="fas fa-check-circle"></i> Configured</div>
            <div class="ak-green text-2xl font-bold text-emerald-300 mt-1">{{ $configured }}</div>
        </div>
        <div class="glass rounded-xl p-4 border border-amber-500/20">
            <div class="ak-amber text-[11px] uppercase tracking-wider text-amber-300/70 flex items-center gap-2"><i class="fas fa-triangle-exclamation"></i> Need setup</div>
            <div class="ak-amber text-2xl font-bold text-amber-300 mt-1">{{ $unconfigured }}</div>
        </div>
    </div>

    <div class="glass rounded-2xl border border-white/10 p-5 space-y-3">
        <h2 class="ak-strong text-sm font-semibold text-white">How this works</h2>
        <p class="ak-muted text-xs text-white/50 leading-relaxed">
            ...
            <div class="glass rounded-2xl border {{ $p['configured'] ? 'border-emerald-500/30' : 'border-amber-500/30' }} p-5">
                <div class="flex items-start justify-between gap-4 flex-wrap">
                    <div class="flex items-center gap-3">
                        <div class="ak-strong w-10 h-10 rounded-xl flex items-center justify-center text-white/80 bg-white/5 border border-white/10">
                            <i class="{{ $p['icon'] }}"></i>
                        </div>
                        <div>
                            <div class="ak-strong text-sm font-semibold text-white">{{ $p['label'] }}</div>
                            <div class="ak-note text-[11px] text-white/40 mt-0.5">key: <span class="font-mono">{{ $p['key'] }}</span></div>
                        </div>
                    </div>
                    @if($p['configured'])
                        <span class="ak-green inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
                            <i class="fas fa-check"></i>
                            {{ $p['has_admin_value'] ? 'Configured' : 'Configured (env fallback)' }}
                        </span>
                    @else
                        <span class="ak-amber inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30">
                ...
                <form method="POST" action="{{ route('admin.social-oauth.update', $p['key']) }}" class="mt-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label class="ak-note block text-[10px] uppercase tracking-wider text-white/40 mb-1">Client ID</label>
                            <input type="text" name="client_id" value="{{ old('client_id', $p['admin_client_id']) }}"
                                   ...
                                   class="ak-note w-full rounded-lg bg-white/[0.04] border border-white/10 px-3 py-2 text-sm text-white/90 font-mono placeholder:text-white/25 focus:border-indigo-400/50 focus:outline-none">
                        </div>
                        <div>
                            <label class="ak-note block text-[10px] uppercase tracking-wider text-white/40 mb-1">
                                Client Secret
                                @if($p['has_admin_secret'])
                                    <span class="ak-green text-emerald-300/70 normal-case tracking-normal"> - saved</span>
                                @endif
                            </label>
                            @include('common.partials.password-field', [
                                ...
                                'inputClass' => 'ak-input w-full rounded-lg bg-white/[0.04] border border-white/10 px-3 py-2 text-sm text-white/90 font-mono placeholder:text-white/25 focus:border-indigo-400/50 focus:outline-none',
                            ])
                        </div>
                    </div>

                    <div class="flex items-center justify-between gap-3 flex-wrap">
                        <label class="ak-muted inline-flex items-center gap-2 text-[11px] text-white/50 select-none">
                    ...
                    <p class="ak-note text-[10px] text-white/30 font-mono">env fallback: {{ $p['client_id_env'] }} / {{ $p['client_secret_env'] }}</p>
                </form>

                @if($p['notes'])
                    <p class="ak-muted mt-3 text-xs text-white/55 leading-relaxed">{{ $p['notes'] }}</p>
                @endif

...
<div class="max-w-6xl">
    <div class="glass rounded-2xl border border-white/10 p-6 mb-6">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white/90">Quarantined uploads</h2>
            ...
            <div class="grid grid-cols-3 gap-3 text-center">
                <div class="rounded-xl px-4 py-3 border border-white/10 bg-white/[0.02]">
                    <div class="text-[10px] uppercase font-bold tracking-wider text-white/50">Awaiting review</div>
                    <div class="text-2xl font-bold text-rose-400">{{ number_format($counts['flagged_pending']) }}</div>
                </div>
                <div class="rounded-xl px-4 py-3 border border-white/10 bg-white/[0.02]">
                    <div class="text-[10px] uppercase font-bold tracking-wider text-white/50">Flagged total</div>
                    <div class="text-2xl font-bold text-white">{{ number_format($counts['flagged_total']) }}</div>
                </div>
                <div class="rounded-xl px-4 py-3 border border-white/10 bg-white/[0.02]">
                    <div class="text-[10px] uppercase font-bold tracking-wider text-white/50">Pending scan</div>
                    <div class="text-2xl font-bold text-sky-400">{{ number_format($counts['pending']) }}</div>
                </div>
            </div>
        </div>
        ...
        <form method="GET" class="mt-5 flex items-center gap-3 flex-wrap">
            <div class="flex items-center gap-2">
                <label class="text-[10px] uppercase font-bold tracking-wider text-white/50">Status</label>
            ...
            <div class="flex items-center gap-2">
                <label class="text-[10px] uppercase font-bold tracking-wider text-white/50">Review</label>
    ...
    <div class="glass rounded-2xl border border-white/10 overflow-hidden">
        @if($files->isEmpty())
            <div class="p-10 text-center text-sm text-white/50">
                <i class="fas fa-shield-check text-3xl text-emerald-400 mb-3"></i>
                <div>Nothing in the review queue right now. Nice.</div>
            </div>
        @else
            <table class="w-full text-sm">
                <thead class="text-[10px] uppercase tracking-wider text-white/50 bg-white/[0.03]">
                    <tr>
                    ...
                    @foreach($files as $f)
                        <tr class="border-t border-white/5 hover:bg-white/[0.02]">
                            <td class="px-4 py-3 align-top">
                                <div class="font-mono text-xs text-white/85 truncate max-w-xs" title="{{ $f->original_name }}">
                                    {{ $f->original_name }}
                                </div>
                            ...
                            <td class="px-4 py-3 align-top">
                                <div class="text-xs text-white/85">{{ $f->user->name ?? '—' }}</div>
                                <div class="text-[10px] text-white/40">{{ $f->user->email ?? '—' }}</div>
                            </td>
                            <td class="px-4 py-3 align-top">
                                ...
                                <div class="text-xs text-white/70 mt-1">{{ $reasonLabel($f->scan_reason) }}</div>
                                @if(!empty($f->scan_meta))
                                    <details class="mt-1">
                                        <summary class="text-[10px] text-white/40 cursor-pointer">Details</summary>
                                        <pre class="text-[10px] text-white/60 mt-1 max-w-xs whitespace-pre-wrap break-all">{{ json_encode($f->scan_meta, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                                    </details>
                                @endif
                            </td>
                            <td class="px-4 py-3 align-top text-xs text-white/60">
                                ...
                                @if($f->scan_admin_reviewed)
                                    <div class="text-[10px] text-emerald-400 mt-1"><i class="fas fa-check"></i> Reviewed</div>
                                @endif
                            </td>
                            <td class="px-4 py-3 align-top text-right">
                                <form method="POST" action="{{ route('admin.file-scan-queue.rescan', $f->id) }}" class="inline">@csrf
                                    <button class="px-2.5 py-1 rounded text-[11px] text-white" style="background: rgba(56,189,248,0.25); color: #7dd3fc;">
                                ...
                                @unless($f->scan_admin_reviewed)
                                    <form method="POST" action="{{ route('admin.file-scan-queue.acknowledge', $f->id) }}" class="inline">@csrf
                                        <button class="px-2.5 py-1 rounded text-[11px] text-white" style="background: rgba(16,185,129,0.25); color: #6ee7b7;">

...
<div class="max-w-5xl">
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 mb-6">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white/90">Marketing CTA click counts</h2>
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-white/50">
                        Source: <code class="text-white/70">{{ $section['source'] }}</code>
                    ...
                    <tbody>
                        @foreach($section['rows'] as $r)
                            <tr class="border-b border-white/5">
                                ...
                                <td class="py-2 pr-4 text-white/50"><code>{{ $r['target'] }}</code></td>
                                <td class="py-2 pr-4 text-right text-white/90">{{ number_format($r['count']) }}</td>
                                <td class="py-2 pr-4 text-right text-white/50">
                                    @if($section['total'] > 0)

...
<div class="max-w-3xl space-y-6">
    ...
    <div class="admin-mp-banner p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-200/90 text-sm space-y-1 ak-amber">
        <p class="font-semibold flex items-center gap-2">
            <i class="fas fa-triangle-exclamation"></i> Highly sensitive
        </p>
        <p class="admin-mp-banner-sub text-amber-200/70 text-xs ak-amber">
    ...
    <form method="POST" action="{{ route('admin.master-password.update') }}" class="space-y-6">
        ...
        <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h3 class="font-semibold text-white flex items-center gap-2 ak-strong">
                        <i class="fas fa-key text-blue-400 ak-blue"></i> Master override password
                    </h3>
                    <p class="text-xs text-white/40 ak-note">Off by default. Set a password, then enable the override.</p>
                </div>
                <div class="shrink-0">
            ...
            <div>
                <label class="text-xs uppercase tracking-wider text-white/40 mb-1 block ak-note">
                    {{ $hasPassword ? 'New master password' : 'Master password' }}
                </label>
                @if($hasPassword)
                    <p class="text-xs text-white/60 mb-1 ak-muted">
                        A master password is currently stored
                        <span class="font-mono text-amber-300 ak-amber">••••••••</span>.
            ...
            <label class="inline-flex items-center gap-2 text-sm text-white/80 ak-strong">
            ...
            @if($hasPassword)
                <label class="block mt-2 inline-flex items-center gap-2 text-xs text-white/60 ak-muted">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if($isActive)
                <span class="text-xs text-emerald-300/80 ak-green">
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-4">
        <div>
            <h3 class="font-semibold text-white text-sm flex items-center gap-2 ak-strong">
                <i class="fas fa-list-check text-sky-400 ak-blue"></i> Master-password logins
            </h3>
            <p class="text-xs text-white/40 ak-note">The 50 most recent sign-ins that used the master password.</p>
        </div>

        @if($logins->isEmpty())
            <p class="text-xs text-white/40 ak-note">No master-password logins recorded yet.</p>
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-xs">
                    <thead>
                        <tr class="text-white/40 uppercase tracking-wider text-[10px] text-left ak-note">
                    ...
                    <tbody class="divide-y divide-white/5">
                        @foreach($logins as $login)
                            <tr class="text-white/70 ak-strong">
                                ...
                                <td class="py-2 pr-3">
                                    <span class="text-white/90 ak-strong">{{ $login->target_name ?: '—' }}</span>
                                    <span class="block text-white/40 ak-note">{{ $login->target_email ?: '' }}</span>
                                </td>
                                <td class="py-2 pr-3 whitespace-nowrap">{{ $login->guardLabel() }}</td>
                                <td class="py-2 pr-3 font-mono whitespace-nowrap">{{ $login->ip ?: '—' }}</td>
                                <td class="py-2 pr-3 max-w-[220px] truncate text-white/40 ak-note" title="{{ $login->user_agent }}">
                                    {{ $login->user_agent ?: '—' }}
                                </td>
                            </tr>
                        @endforeach

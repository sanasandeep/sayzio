        ...
        <form method="POST" action="{{ route('admin.staff.update', $staff) }}">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-white/80 mb-1 ak-strong">Password <span class="text-white/30 ak-note">(leave blank to keep)</span></label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" required @if(!empty($isProtected)) disabled @endif class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-blue-500/40 outline-none @if(!empty($isProtected)) opacity-60 cursor-not-allowed @endif ak-strong ak-input">
                        ...
                        <p class="mt-1 text-xs text-emerald-400/80 ak-green"><i class="fas fa-shield-alt mr-1"></i> Protected account, cannot be deactivated.</p>
                        @endif
                    </div>
                </div>

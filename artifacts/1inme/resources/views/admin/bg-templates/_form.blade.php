...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <p class="text-[11px] text-white/40 mt-1.5">
            Used in the CSS selector <code class="text-white/60">.bg-template-&lt;slug&gt;</code>. Lowercase, dashes only.
        </p>
    </div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div>
        <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5">Category</label>
    ...
    <div>
        <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5">Status</label>
        <label class="flex items-center gap-2 cursor-pointer h-[42px] px-3 rounded-lg bg-black/30 border border-white/15">
            ...
            <input type="checkbox" name="is_active" value="1"
                   ...
                   class="rounded">
            <span class="text-sm text-white/80">Active &amp; visible to users</span>
        </label>
    </div>
</div>
...
<div>
    <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5">
        Preview swatch <span class="text-white/30 normal-case">(any CSS background value)</span>
    </label>
...
<div>
    <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5">CSS</label>
    ...
    <p class="text-[11px] text-white/40 mt-1.5">
        Scope every rule under <code class="text-white/60">.bg-template-&lt;slug&gt;</code> and keep the wrapper at
        <code class="text-white/60">position:fixed; inset:0; z-index:-1</code> so it sits behind biolink content.
    </p>
</div>
...
<div>
    ...
    <p class="text-[11px] text-white/40 mt-1.5">
        Runs inside an IIFE after page load. <code class="text-white/60">container</code> is the
        <code class="text-white/60">.bg-template-&lt;slug&gt;</code> element.
    </p>
</div>

@if(!empty($template->slug))
<div>
    <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5">Live preview</label>

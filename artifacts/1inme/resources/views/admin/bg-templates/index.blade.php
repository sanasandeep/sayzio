...
<div class="max-w-7xl space-y-6">
    <div class="glass rounded-2xl border border-white/10 p-6">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white/90 ak-strong">Link in Bio background templates</h2>
                <p class="text-xs text-white/50 mt-1 max-w-2xl ak-muted">
                    ...
                    Each template renders as <code class="text-white/60 ak-muted">.bg-template-&lt;slug&gt;</code>. Toggle
            ...
            <div class="flex items-center gap-2">
                <form method="POST" action="{{ route('admin.bg-templates.restore-defaults') }}"
                    ...
                    @csrf
                    <button type="submit"
                            class="px-4 py-2 rounded-xl text-sm font-medium bg-white/10 hover:bg-white/15 text-white/90 inline-flex items-center gap-2 ak-strong">
    ...
    <form method="GET" action="{{ route('admin.bg-templates.index') }}"
        ...
        <div class="flex items-center gap-1.5 flex-wrap">
            <a href="{{ route('admin.bg-templates.index', ['q' => $q]) }}"
               class="text-[12px] font-semibold px-3 py-1.5 rounded-full {{ $currentCat === '' ? 'bg-blue-600/30 text-blue-200 border border-blue-500/40 ak-blue' : 'bg-white/5 text-white/70 border border-white/10 ak-strong' }}">
                All <span class="opacity-60">{{ $totalCount }}</span>
            </a>
            @foreach($categories as $cat)
                <a href="{{ route('admin.bg-templates.index', ['q' => $q, 'cat' => $cat]) }}"
                   class="text-[12px] font-semibold px-3 py-1.5 rounded-full {{ $currentCat === $cat ? 'bg-blue-600/30 text-blue-200 border border-blue-500/40 ak-blue' : 'bg-white/5 text-white/70 border border-white/10 ak-strong' }}">
        ...
        @foreach($templates as $tpl)
            <div class="glass rounded-2xl border {{ $tpl->is_active ? 'border-white/10' : 'border-amber-500/30' }} p-3 flex flex-col gap-2">
                <div class="rounded-xl overflow-hidden relative" style="aspect-ratio: 9/16; background: {{ $tpl->preview_color }};">
                    ...
                    @endif
                    <div class="absolute bottom-1.5 right-1.5 text-[10px] px-1.5 py-0.5 rounded-md bg-black/50 text-white/80 backdrop-blur ak-strong">{{ $tpl->category }}</div>
                </div>
                <div>
                    <p class="text-sm font-semibold text-white/90 truncate ak-strong" title="{{ $tpl->name }}">{{ $tpl->name }}</p>
                    <p class="text-[10px] text-white/40 font-mono truncate ak-note">{{ $tpl->slug }}</p>
                </div>

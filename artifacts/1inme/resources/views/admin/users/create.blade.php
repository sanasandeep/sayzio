...
<div class="max-w-2xl">
    <div class="glass rounded-2xl border border-white/10 p-6">
        <h3 class="text-lg font-semibold text-white mb-1 ak-strong">Create a new account</h3>
        <p class="text-xs text-white/40 mb-6 ak-note">Provision a user with an initial plan and optional starting coins. Leave the password blank to auto-generate one and email sign-in credentials.</p>

        @if($errors->any())
        <div class="mb-4 rounded-xl border border-rose-500/30 bg-rose-500/10 p-3 text-sm text-rose-200 ak-red">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-white/80 mb-1 ak-strong">Handle <span class="text-white/40 ak-note">(optional)</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-white/80 mb-1 ak-strong">Starting coins <span class="text-white/40 ak-note">(optional)</span></label>
                    <input type="number" name="starting_coins" value="{{ old('starting_coins') }}" min="1" max="1000000" placeholder="0"
                           class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/30 focus:ring-2 focus:ring-blue-500/40 outline-none ak-strong ak-input">
                </div>
                <div>
                    <label class="block text-sm font-medium text-white/80 mb-1 ak-strong">Password <span class="text-white/40 ak-note">(blank = auto-generate)</span></label>
                    <input type="text" name="password" autocomplete="off" placeholder="Min 8 chars, or leave blank"
                           class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/30 focus:ring-2 focus:ring-blue-500/40 outline-none ak-strong ak-input">
                </div>
            </div>

            <label class="flex items-center gap-2 text-sm text-white/70 ak-strong">
            ...
            <div class="rounded-xl border border-white/5 bg-white/[0.02] p-4 space-y-3">
                ...
                <div x-show="createStaff" x-cloak>
                    <label class="block text-sm font-medium text-white/80 mb-1 ak-strong">Staff role</label>
                    <select name="role_id"
                            class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-blue-500/40 outline-none ak-strong ak-input">
            ...
            <div class="flex items-center gap-3 pt-2">
                ...
                <a href="{{ route('admin.users.index') }}" class="px-4 py-2.5 text-white/60 hover:text-white/90 text-sm ak-muted">Cancel</a>
            </div>
        </form>
    </div>
</div>

...
<div class="space-y-6">
    <div class="flex items-start justify-between gap-3 flex-wrap">
        <div>
            <h2 class="text-lg font-semibold text-white ak-strong">Role change audit log</h2>
    ...
    <form method="GET" action="{{ route('admin.users.role-audits.index') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">Actor</span>
                <input type="text" name="actor" value="{{ $filters['actor'] }}"
                       placeholder="Name, email, or id"
                       class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            </label>
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">Target user</span>
                <input type="text" name="target" value="{{ $filters['target'] }}"
                       placeholder="Name, email, or id"
                       class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            </label>
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">Role</span>
                <select name="role"
                        class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            ...
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">Action</span>
                <select name="action"
                        class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            ...
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">Source</span>
                <select name="source"
                        class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            ...
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">From</span>
                <input type="date" name="from" value="{{ $filters['from'] }}"
                       class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 ak-strong ak-input">
            </label>
            <label class="block">
                <span class="block text-xs uppercase tracking-wide text-white/40 mb-1 ak-note">To</span>
        ...
        <div class="flex flex-wrap items-center gap-2 pt-1">
            ...
            @if($hasAnyFilter)
                <a href="{{ route('admin.users.role-audits.index') }}"
                   class="px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 text-sm ak-muted">Clear</a>
            @endif
            <span class="flex-1"></span>
            <a href="{{ route('admin.users.role-audits.export', $filters) }}"
               class="px-4 py-2 rounded-xl bg-emerald-500/20 text-emerald-200 hover:bg-emerald-500/30 text-sm font-medium ak-green">
                    ...
                    @forelse($audits as $a)
                        <tr>
                            <td class="px-4 py-2 text-xs text-white/60 whitespace-nowrap ak-muted"
                                ...
                                <div class="text-[10px] text-white/30 ak-note">{{ $a->created_at?->diffForHumans() }}</div>
                            </td>
                            <td class="px-4 py-2 text-white ak-strong">
                                {{ $a->actorLabel() }}
                                @if($a->actor_email)
                                    <div class="text-[11px] text-white/40 ak-note">{{ $a->actor_email }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-2">
                            ...
                            <td class="px-4 py-2">
                                <div class="text-white ak-strong">{{ $a->role_name ?: $a->role_slug }}</div>
                                @if($a->role_name && $a->role_slug && $a->role_name !== $a->role_slug)
                                    <div class="text-[11px] text-white/40 ak-note">{{ $a->role_slug }}</div>
                                @endif
                            </td>
                            <td class="px-4 py-2">
                                @if($a->targetUser)
                                    ...
                                    @if($a->targetUser->email)
                                        <div class="text-[11px] text-white/40 ak-note">{{ $a->targetUser->email }}</div>
                                    @endif
                                @else
                                    <span class="text-white/60 ak-muted">User #{{ $a->target_user_id }}</span>
                                @endif
                            </td>
                            <td class="px-4 py-2 text-xs text-white/60 ak-muted">
                                {{ $sources[$a->source] ?? ($a->source ?: '—') }}
                            </td>
                            <td class="px-4 py-2 text-xs text-white/50 font-mono ak-muted">{{ $a->ip ?: '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-white/40 ak-note">
                                @if($hasAnyFilter)

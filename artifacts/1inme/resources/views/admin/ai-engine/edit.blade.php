...
<form method="POST" action="{{ route('admin.ai-engine.update') }}" class="max-w-5xl space-y-6">
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
        <div>
            <h3 class="ak-strong font-semibold text-white">Engine</h3>
            <p class="ak-note text-xs text-white/40">Master switch for the entire AI subsystem and API credentials.</p>
        </div>
        ...
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <span class="ak-strong text-sm text-white">Enable AI Engine</span>
        </label>
        ...
        <div>
            <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">OpenAI API key</label>
            @if($hasKey)
                <p class="ak-muted text-xs text-white/60 mb-2">Stored: <span class="ak-amber font-mono text-amber-300">{{ $maskedKey }}</span></p>
            @endif
        ...
        <div class="pt-4 border-t border-white/10 space-y-3">
            <div class="flex flex-wrap items-center gap-3">
                <button type="button" onclick="testOpenAiConnection(this)"
                        class="ak-strong px-3 py-1.5 bg-white/10 hover:bg-white/20 border border-white/15 text-white rounded-lg text-xs font-medium">
                    <i class="fas fa-plug mr-1"></i> Test connection
                </button>
                <span id="openai-test-result" class="ak-muted text-xs text-white/50"></span>
            </div>
            <p class="ak-note text-[11px] text-white/30">Sends a tiny 1-token request to OpenAI to confirm the key works. No coins are charged. Tests the key typed above, or the stored key when the field is blank.</p>

            <div class="flex flex-wrap items-center gap-4 pt-1">
                <a href="{{ route('admin.ai-usage.index') }}" class="ak-blue text-xs text-blue-300 hover:underline">
                ...
                <a href="https://platform.openai.com/settings/organization/billing/overview" target="_blank" rel="noopener"
                   class="ak-blue text-xs text-blue-300 hover:underline">
                    <i class="fas fa-arrow-up-right-from-square mr-1"></i> Check OpenAI balance
                </a>
            </div>
            <p class="ak-note text-[11px] text-white/30">Live USD balance lives on OpenAI's side, open their billing dashboard to check it. Internal token &amp; coin consumption is in the AI usage report.</p>
        </div>
    </div>
            ...
            @foreach($models as $i => $m)
                ...
                <tr class="border-t border-white/5 align-top" data-model-row data-features="{{ implode(',', $usedBy) }}">
                        ...
                        @if($usedBy)
                            <div class="flex flex-wrap gap-1">
                                @foreach($usedBy as $feat)
                                    <span class="ak-blue inline-flex items-center px-1.5 py-0.5 rounded bg-blue-500/15 border border-blue-500/30 text-blue-200 text-[10px] font-mono uppercase tracking-wider">{{ $feat }}</span>
                                @endforeach
                            </div>
                        @else
                            <span class="ak-note text-white/30 text-xs">-</span>
                        @endif
                    </td>
                    <td><select name="models[{{ $i }}][kind]" class="ak-strong ak-input bg-white/5 border border-white/10 rounded px-2 py-1 text-white text-sm">
                    ...
                    <td>
                        ...
                        <p data-disable-warning class="ak-amber hidden mt-1 text-[11px] text-amber-300 flex items-start gap-1">
                            <i class="fas fa-triangle-exclamation mt-0.5"></i>
                            <span data-disable-warning-text></span>
                        </p>
                    </td>
                    <td class="text-right"><input type="number" min="0" step="0.01" name="models[{{ $i }}][in_coins_per_1k]" value="{{ $m['in_coins_per_1k'] }}" class="ak-strong ak-input w-24 text-right bg-white/5 border border-white/10 rounded px-2 py-1 text-white text-sm"></td>
                    <td class="text-right"><input type="number" min="0" step="0.01" name="models[{{ $i }}][out_coins_per_1k]" value="{{ $m['out_coins_per_1k'] }}" class="ak-strong ak-input w-24 text-right bg-white/5 border border-white/10 rounded px-2 py-1 text-white text-sm"></td>
                    <td class="text-right"><button type="button" onclick="this.closest('tr').remove()" class="ak-note text-white/30 hover:text-red-400"><i class="fas fa-trash"></i></button></td>
                </tr>
            @endforeach
            </tbody>
        </table>
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-3">
        <div>
            <h3 class="ak-strong font-semibold text-white">Feature model history</h3>
            <p class="ak-note text-xs text-white/40">
        ...
        @if(count($featureModelHistory) === 0)
            <p class="ak-note text-xs text-white/40 italic">No changes recorded yet.</p>
        @else
            <table class="w-full text-sm">
                <thead><tr class="ak-note text-white/40 text-xs uppercase tracking-wider">
                ...
                @foreach($featureModelHistory as $row)
                    <tr class="border-t border-white/5 align-top">
                        <td class="ak-strong py-2 text-white/70 whitespace-nowrap" title="{{ $row->created_at?->toDateTimeString() }}">
                            {{ $row->created_at?->diffForHumans() }}
                        </td>
                        <td class="ak-strong text-white/80 font-mono">{{ $row->feature }}</td>
                        <td class="ak-muted text-white/60 font-mono">{{ $row->old_model ?? '—' }}</td>
                        <td class="ak-amber text-amber-300 font-mono">{{ $row->new_model ?? '—' }}</td>
                        <td class="ak-muted text-white/60">
    ...
    <div class="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-5">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h3 class="ak-strong text-white text-base font-semibold">AI Voice Assistant</h3>
                <p class="ak-muted text-white/50 text-xs mt-1">Whisper transcribes, GPT reasons &amp; calls tools, ElevenLabs speaks. Each stage is metered separately and charged from the coin wallet.</p>
            </div>
            <label class="inline-flex items-center gap-2 cursor-pointer">
                ...
                <input type="checkbox" name="voice_enabled" value="1" {{ $voiceEnabled ? 'checked' : '' }}
                       class="ak-blue ak-input h-4 w-4 rounded border-white/20 bg-white/5 text-blue-500 focus:ring-blue-500">
                <span class="ak-strong text-white/80 text-sm">Enabled</span>
            </label>
        </div>

        <div class="grid md:grid-cols-2 gap-4">
            <div>
                <label class="ak-strong text-white/70 text-xs">Whisper API key</label>
                @if($hasWhisperKey)
                    <div class="flex items-center gap-2 mt-1">
                        <code class="ak-muted text-white/60 text-xs bg-white/5 border border-white/10 rounded px-2 py-1 flex-1">{{ $maskedWhisperKey }}</code>
                        <label class="ak-red inline-flex items-center gap-1 text-xs text-red-300"><input type="checkbox" name="clear_whisper_api_key" value="1"> clear</label>
                    </div>
                @endif
                @include('common.partials.password-field', [
                    ...
                    'wrapClass' => 'mt-1',
                    'inputClass' => 'ak-input w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm',
                ])
                <div class="flex flex-wrap items-center gap-2 mt-2">
                    ...
                    <span id="whisper-test-result" class="ak-muted text-xs text-white/50"></span>
                </div>
                <p class="ak-note text-white/40 text-[11px] mt-1">Falls back to the main OpenAI key when blank. Test lists OpenAI models to validate the key, no coins charged.</p>
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">Whisper model</label>
                <input type="text" name="whisper_model" value="{{ $whisperModel }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">Voice GPT model (tool-calling)</label>
                <input type="text" name="voice_gpt_model" value="{{ $voiceGptModel }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
                <p class="ak-note text-white/40 text-[11px] mt-1">Cost is billed under <code>voice_llm</code> using the model's per-1k token rates.</p>
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">ElevenLabs API key</label>
                @if($hasElevenKey)
                    <div class="flex items-center gap-2 mt-1">
                        <code class="ak-muted text-white/60 text-xs bg-white/5 border border-white/10 rounded px-2 py-1 flex-1">{{ $maskedElevenKey }}</code>
                        <label class="ak-red inline-flex items-center gap-1 text-xs text-red-300"><input type="checkbox" name="clear_elevenlabs_api_key" value="1"> clear</label>
                    </div>
                @endif
                @include('common.partials.password-field', [
                ...
                <div class="flex flex-wrap items-center gap-2 mt-2">
                    ...
                    <span id="elevenlabs-test-result" class="ak-muted text-xs text-white/50"></span>
                </div>
                <p class="ak-note text-white/40 text-[11px] mt-1">Checks the ElevenLabs voices endpoint to validate the key, no coins charged.</p>
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">ElevenLabs voice id</label>
                <input type="text" name="elevenlabs_voice_id" value="{{ $elevenVoiceId }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">ElevenLabs model</label>
                <input type="text" name="elevenlabs_model" value="{{ $elevenModel }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">STT coins per minute of audio</label>
                <input type="number" min="0" step="0.01" name="voice_price_stt" value="{{ $voicePriceStt }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">TTS coins per 1 000 characters</label>
                <input type="number" min="0" step="0.01" name="voice_price_tts" value="{{ $voicePriceTts }}"
                       class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            </div>
            <div>
                <label class="ak-strong text-white/70 text-xs">Rate limit (turns / user / minute)</label>
        ...
        @if($allPlans->count())
            <div>
                <label class="ak-strong text-white/70 text-xs">Allow Voice on plans <span class="ak-note text-white/40">(none ticked = all plans)</span></label>
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
        <div>
            <h3 class="ak-strong font-semibold text-white">AI Artistic QR</h3>
            <p class="ak-note text-xs text-white/40">
        ...
        <div>
            <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Replicate API token</label>
            @if($hasStoredReplicateKey)
                <p class="ak-muted text-xs text-white/60 mb-2">Stored: <span class="ak-amber font-mono text-amber-300">{{ $maskedReplicateKey }}</span></p>
            @elseif($hasReplicateKey)
                <p class="ak-muted text-xs text-white/60 mb-2">Using environment token <span class="ak-amber font-mono text-amber-300">{{ $maskedReplicateKey }}</span> (<code>REPLICATE_API_TOKEN</code>).</p>
            @else
                <p class="ak-amber text-xs text-amber-300/80 mb-2">No token configured, AI Artistic QR is in preview / disabled mode.</p>
            @endif
            @include('common.partials.password-field', [
            ...
            @endif
            <p class="ak-note text-[11px] text-white/30 mt-1">Encrypted at rest. Falls back to <code>REPLICATE_API_TOKEN</code> when no token is stored here.</p>
        </div>
        ...
        <div class="pt-4 border-t border-white/10 space-y-3">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <span id="replicate-test-result" class="ak-muted text-xs text-white/50"></span>
            </div>
            <p class="ak-note text-[11px] text-white/30">Makes a lightweight authenticated call to Replicate to confirm the token works. No prediction is queued and no coins are charged. Tests the token typed above, or the stored/environment token when the field is blank.</p>
        </div>

        <div class="pt-4 border-t border-white/10">
            <label class="ak-strong text-white/70 text-xs">Coins charged per generation</label>
            <input type="number" min="1" max="100000" step="1" name="qr_art_coins" value="{{ $qrArtCoins }}"
                   class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            <p class="ak-note text-[11px] text-white/30 mt-1">Flat coin price billed from the user's wallet for each AI Artistic QR (auto-refunded if generation fails).</p>
        </div>

        <div class="pt-4 border-t border-white/10">
            <label class="ak-strong text-white/70 text-xs">Brand asset base coins per generation</label>
            <input type="number" min="1" max="100000" step="1" name="brand_asset_coins" value="{{ $brandAssetCoins }}"
                   class="ak-strong ak-input mt-1 w-full bg-white/5 border border-white/10 rounded px-3 py-2 text-white text-sm">
            <p class="ak-note text-[11px] text-white/30 mt-1">Base coin price for each Brand Kit visual asset (logo, favicon, banners…). The final charge scales by asset size and the user's plan multiplier; auto-refunded if generation fails. Uses the OpenAI key above.</p>
        </div>
    </div>
...
async function testOpenAiConnection(btn) {
    ...
    try {
        ...
        result.className = 'text-xs ' + (data.ok ? 'ak-green text-emerald-300' : 'ak-red text-red-300');
...
function addModelRow() {
    ...
    row.innerHTML = `
        ...
        <td class="text-right"><input type="number" min="0" step="0.01" name="models[${i}][out_coins_per_1k]" value="0" class="ak-strong ak-input w-24 text-right bg-white/5 border border-white/10 rounded px-2 py-1 text-white text-sm"></td>
        <td class="text-right"><button type="button" onclick="this.closest('tr').remove()" class="ak-note text-white/30 hover:text-red-400"><i class="fas fa-trash"></i></button></td>`;
    tb.appendChild(row);
}
...
(function () {
    ...
    function renderBadges(row, features) {
        ...
        if (!cell) return;
        if (!features.length) {
            cell.innerHTML = '<span class="ak-note text-white/30 text-xs">-</span>';
            return;
        }
        cell.innerHTML = '<div class="flex flex-wrap gap-1">' + features.map(function (f) {
            return '<span class="ak-blue inline-flex items-center px-1.5 py-0.5 rounded bg-blue-500/15 border border-blue-500/30 text-blue-200 text-[10px] font-mono uppercase tracking-wider">' + f + '</span>';
        }).join('') + '</div>';
    }

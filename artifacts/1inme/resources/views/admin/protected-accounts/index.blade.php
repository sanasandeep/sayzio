...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 mb-6">
        <h3 class="text-lg font-semibold text-white mb-1 ak-strong">Add protected account</h3>
        <p class="text-xs text-white/40 mb-4 ak-note">Protects both the user dashboard and any matching back-office staff account (matched by email). For accounts without an email (e.g. WhatsApp-only signups), enter the user ID instead.</p>
        <form method="POST" action="{{ route('admin.protected-accounts.store') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <input type="number" name="user_id" min="1" placeholder="or user ID"
                   class="w-full sm:w-36 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/30 focus:ring-2 focus:ring-blue-500/40 outline-none ak-strong ak-input">
    ...
    <div class="mb-6 flex items-start gap-3 p-4 rounded-2xl bg-white/5 border border-white/10">
        ...
        <div class="text-sm text-white/60 ak-muted">You can view the protected list, but only a superadmin can add or remove accounts.</div>
    </div>
    @endif
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-b border-white/5">
                    <td class="px-6 py-4 text-sm text-white ak-strong">
                        @if($account->email)
                            {{ $account->email }}
                        @else
                            @php($protectedUser = isset($usersById) ? $usersById->get($account->user_id) : null)
                            <span class="inline-flex items-center gap-2">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-white/10 text-white/60 ak-muted">User #{{ $account->user_id }}</span>
                                @if($protectedUser)
                                    <span>{{ $protectedUser->name ?? $protectedUser->handle ?? '—' }}</span>
                                @else
                                    <span class="text-white/40 ak-note">(account no longer exists)</span>
                                @endif
                            </span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-sm text-white/60 ak-muted">{{ $account->label ?? '—' }}</td>
                    <td class="px-6 py-4">
                        @if($account->isLocked())
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-500/10 text-blue-300 ak-blue">
                                <i class="fas fa-lock text-[10px]"></i> Permanent
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 ak-green">
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        <form action="{{ route('admin.protected-accounts.destroy', $account) }}" method="POST" class="inline"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit" class="text-white/30 hover:text-red-400 ak-note" title="Remove protection"><i class="fas fa-trash"></i></button>
                        </form>
                        @elseif($account->isLocked())
                        <span class="text-white/20 ak-note" title="Cannot be removed"><i class="fas fa-lock"></i></span>
                        @else
                ...
                @empty
                <tr><td colspan="4" class="px-6 py-8 text-center text-white/30 ak-note">No protected accounts</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

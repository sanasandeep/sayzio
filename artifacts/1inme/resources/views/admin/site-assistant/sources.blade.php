        ...
        <form method="POST" action="{{ route('admin.site-assistant.sources.store') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" maxlength="200" required value="{{ old('title') }}" class="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g. Pricing FAQ">
                </div>
            </div>
            <div id="sa-source-url-row">
                <label class="block text-xs text-white/60 mb-1">URL</label>
                <input type="url" name="url" maxlength="2048" value="{{ old('url') }}" class="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="https://example.com/help/pricing">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-xs text-white/60 mb-1">Page pattern (optional)</label>
                    <input type="text" name="page_pattern" maxlength="200" value="{{ old('page_pattern') }}" class="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white" placeholder="marketing.pricing or /pricing*">
                </div>
                <div>
                    <label class="block text-xs text-white/60 mb-1">Surface</label>
                    <select name="assistant_surface" class="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white">
                ...
                <div>
                    <label class="block text-xs text-white/60 mb-1">URL refresh (minutes, links only)</label>
                    <input type="number" name="refresh_minutes" min="15" max="43200" value="{{ old('refresh_minutes', 1440) }}" class="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white">
                </div>
            </div>
            <div>
                    ...
                    <tr id="source-{{ $s->id }}" class="border-t border-white/5 align-top target:bg-indigo-500/10">
                        <td class="p-3 text-white">
                            ...
                            @if($s->type === 'link' && $s->url)
                                <div class="text-xs text-white/40 truncate max-w-xs"><a class="hover:text-white" target="_blank" href="{{ $s->url }}">{{ $s->url }}</a></div>
                        ...
                        <td class="p-3 text-white/70">
                            <span class="text-[10px] uppercase tracking-wide rounded px-1.5 py-0.5
                                @if($s->status === 'ready') bg-emerald-500/20 text-emerald-200
                                @elseif($s->status === 'failed') bg-red-500/20 text-red-200
                                @else bg-white/10 text-white/70 @endif">{{ $s->status }}</span>

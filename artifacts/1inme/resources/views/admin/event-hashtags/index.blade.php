...
<div class="max-w-4xl space-y-6">
    <div class="glass rounded-2xl border border-white/10 p-6">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white/90">/events hashtag row</h2>
                <p class="text-xs text-white/50 mt-1 max-w-2xl">
                    These predefined hashtags show first on the public <code class="text-white/60">/events</code> page,
    ...
    <div class="glass rounded-2xl border border-white/10 overflow-hidden">
            ...
            <tbody>
                @foreach($hashtags as $i => $tag)
                    <tr class="border-b border-white/5 last:border-0">
                            ...
                            <div class="flex items-center gap-1">
                                <form method="POST" action="{{ route('admin.event-hashtags.move', $tag) }}">
                                    ...
                                    <button type="submit" @if($i === 0) disabled @endif
                                            class="w-6 h-6 rounded-md bg-white/5 hover:bg-white/10 text-white/70 disabled:opacity-25 disabled:cursor-not-allowed">
                        ...
                        <td class="px-4 py-3 font-mono text-white/90">#{{ $tag->tag }}</td>
                        <td class="px-4 py-3">
        ...
        @if($hashtags->isEmpty())
            <div class="p-8 text-center text-white/60 text-sm">No predefined hashtags yet.</div>
        @endif
    </div>
</div>
@endsection

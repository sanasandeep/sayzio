...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="glass rounded-2xl p-6">
        <h1 class="text-xl font-semibold text-white ak-strong">Free plan renewal reminders</h1>
    ...
    <div class="glass rounded-2xl p-6 space-y-4">
        <div>
            <h2 class="text-base font-semibold text-white ak-strong">Who's due</h2>
            <p class="text-xs text-white/50 ak-muted">
                Free-Starter users with an email and a tracked yearly window{!! $stats['defaultPlanName'] ? ' (plan: <span class="text-white/70 ak-strong">'.e($stats['defaultPlanName']).'</span>)' : '' !!}.
            </p>
        </div>

        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            <div class="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div class="text-2xl font-bold text-amber-300 ak-amber">{{ number_format($stats['dueNext30Days']) }}</div>
                <div class="text-xs text-white/60 mt-1 ak-muted">Due for renewal in next 30 days</div>
                <div class="text-[11px] text-white/40 mt-0.5 ak-note">Free windows lapsing soon.</div>
            </div>

            <div class="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div class="text-2xl font-bold text-white ak-strong">{{ number_format($stats['onStarter']) }}</div>
                <div class="text-xs text-white/60 mt-1 ak-muted">Free-Starter users tracked</div>
                <div class="text-[11px] text-white/40 mt-0.5 ak-note">The pool reminders can target.</div>
            </div>

            <div class="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div class="text-2xl font-bold text-white ak-strong">{{ number_format($stats['lapsed']) }}</div>
                <div class="text-xs text-white/60 mt-1 ak-muted">Windows already lapsed</div>
                <div class="text-[11px] text-white/40 mt-0.5 ak-note">Still active, just awaiting re-confirmation.</div>
            </div>

            <div class="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div class="text-2xl font-bold text-white ak-strong">{{ number_format($stats['remindedLast30Days']) }}</div>
                <div class="text-xs text-white/60 mt-1 ak-muted">Reminded in last 30 days</div>
                <div class="text-[11px] text-white/40 mt-0.5 ak-note">Distinct users, by their window stamp.</div>
            </div>
    ...
    <div class="glass rounded-2xl p-6 space-y-4">
        <div>
            <h2 class="text-base font-semibold text-white ak-strong">Users due for renewal</h2>
        ...
        @if($dueUsers->total() > 0)
            <div class="overflow-x-auto rounded-xl border border-white/10">
                <table class="w-full text-sm text-left">
                    <thead class="text-[11px] uppercase tracking-wide text-white/40 bg-white/[0.03] ak-note">
                        <tr>
                    ...
                    <tbody class="divide-y divide-white/[0.06]">
                        @foreach($dueUsers as $due)
                            ...
                            <tr class="hover:bg-white/[0.02]">
                                <td class="px-4 py-3">
                                    <div class="font-medium text-white truncate max-w-[220px] ak-strong">{{ $due->name ?: 'Unnamed user' }}</div>
                                    <div class="text-xs text-white/50 truncate max-w-[220px] ak-muted">{{ $due->email }}</div>
                                </td>
                                <td class="px-4 py-3">
                                    @if($endsAt)
                                        <div class="text-white/80 ak-strong">{{ $endsAt->format('M j, Y') }}</div>
                                        <div class="text-[11px] {{ $isLapsed ? 'text-red-300 ak-red' : 'text-amber-300 ak-amber' }}">
                                ...
                                <td class="px-4 py-3">
                                    @if($due->starter_renewal_reminder_sent_at)
                                        <div class="text-white/70 ak-strong">{{ $due->starter_renewal_reminder_sent_at->format('M j, Y') }}</div>
                                        <div class="text-[11px] text-white/40 ak-note">{{ $due->starter_renewal_reminder_sent_at->diffForHumans() }}</div>
                                    @else
                                        <span class="text-white/40 ak-note">Never</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <form method="POST" action="{{ route('admin.starter-renewals.users.send', $due->id) }}">
                                        @csrf
                                        <button type="submit"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/15 hover:bg-blue-500/25 border border-blue-400/30 text-blue-200 text-xs font-semibold transition ak-blue">
        ...
        @else
            <div class="rounded-xl border border-white/10 bg-white/[0.03] p-6 text-center text-sm text-white/50 ak-muted">
    ...
    <div class="glass rounded-2xl p-6 space-y-4">
        <div>
            <h2 class="text-base font-semibold text-white ak-strong">Preview a specific user</h2>
            <p class="text-xs text-white/50 mt-0.5 ak-muted">
        ...
        <form method="GET" action="{{ route('admin.starter-renewals.index') }}" class="flex flex-wrap items-center gap-2">
            <input type="text" name="q" value="{{ $search }}"
                   ...
                   class="flex-1 min-w-[220px] px-3 py-2 rounded-lg bg-white/[0.04] border border-white/15 text-sm text-white placeholder-white/30 focus:outline-none focus:border-blue-400/60 ak-strong">
            <button type="submit"
                    class="px-4 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 text-blue-100 text-sm font-semibold transition ak-blue">
                <i class="fas fa-magnifying-glass mr-1.5"></i> Preview this user
            </button>
            @if($search !== '')
                <a href="{{ route('admin.starter-renewals.index') }}"
                   class="px-4 py-2 rounded-lg bg-white/[0.04] hover:bg-white/10 border border-white/15 text-white/70 text-sm font-semibold transition ak-strong">
                    Clear
                </a>
            @endif
        </form>
        ...
        @if($selectedUser)
            <div class="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-3 text-sm text-emerald-100 ak-green">
                <i class="fas fa-circle-check mr-1.5"></i>
                Previewing the reminder for
                <strong>{{ $selectedUser->name ?: 'this user' }}</strong>
                (<span class="text-emerald-200/90 ak-green">{{ $selectedUser->email }}</span>, ID {{ $selectedUser->id }}).
                @if($selectedEndsAt)
                    Free window ends <strong>{{ $selectedEndsAt->format('F j, Y') }}</strong>.
                @endif
            </div>
        @elseif($searchNotFound)
            <div class="rounded-xl border border-amber-400/30 bg-amber-500/10 p-3 text-sm text-amber-100 ak-amber">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div>
                <h2 class="text-base font-semibold text-white ak-strong">Email preview</h2>
                <p class="text-xs text-white/50 mt-0.5 ak-muted">
                    @if($selectedUser)
        ...
        <div class="glass rounded-2xl p-6 space-y-4">
            <div>
                <h2 class="text-base font-semibold text-white ak-strong">In-app notification preview</h2>
                <p class="text-xs text-white/50 mt-0.5 ak-muted">How the nudge appears in a user's notifications feed.</p>
            </div>
    ...
    <div class="glass rounded-2xl p-6 space-y-4">
        <div>
            <h2 class="text-base font-semibold text-white ak-strong">Send yourself a test</h2>
            <p class="text-xs text-white/50 mt-0.5 ak-muted">

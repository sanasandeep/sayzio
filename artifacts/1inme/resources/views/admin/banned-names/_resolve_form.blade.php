...
<form method="POST" action="{{ route('admin.banned-names.conflicts.resolve', $item) }}"
    ...
    <input type="text" name="new_value"
           ...
           pattern="[A-Za-z0-9_\-]+"
           maxlength="100"
           class="px-2.5 py-1.5 rounded-lg text-xs bg-white/5 border border-white/10 text-white/90 placeholder-white/30 font-mono focus:outline-none focus:border-violet-500/50">
    <button type="submit" name="action" value="rename"
            class="px-2.5 py-1.5 rounded-lg text-xs bg-violet-600 hover:bg-violet-700 text-white inline-flex items-center gap-1.5">

        ...
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                <a href="{{ route('admin.banned-names.index') }}"
                   class="text-xs text-white/40 hover:text-white/70 inline-flex items-center gap-1 ak-note">
                    <i class="fas fa-arrow-left text-[10px]"></i> Back to banned names
                </a>
                <h2 class="text-lg font-semibold text-white/90 mt-2 ak-strong">
                    Existing matches for <span class="font-mono text-amber-200 ak-amber">{{ $item->name }}</span>
                </h2>
            ...
            <form method="POST" action="{{ route('admin.banned-names.toggle-force-rename', $item) }}">
                ...
                @if($item->force_rename_on_login)
                    <button type="submit"
                            class="px-3 py-2 rounded-xl text-xs font-medium bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-200 border border-emerald-500/30 inline-flex items-center gap-2 ak-green"
                ...
                @else
                    <button type="submit"
                            class="px-3 py-2 rounded-xl text-xs font-medium bg-white/5 hover:bg-white/10 text-white/70 border border-white/10 inline-flex items-center gap-2 ak-strong"
    ...
    <div class="glass rounded-2xl border border-white/10 overflow-hidden">
        @if(empty($rows))
            <div class="px-6 py-12 text-center text-sm text-white/40 ak-note">
                <i class="fas fa-circle-check text-2xl text-emerald-400/40 mb-3 ak-green"></i>
            ...
            <table class="w-full text-sm">
                ...
                @foreach($rows as $row)
                    <tr class="border-t border-white/5 align-top {{ $row['acknowledged'] ? 'opacity-60' : '' }}">
                        <td class="px-5 py-3">
                            @if($row['kind'] === 'user')
                                <span class="text-xs px-2 py-0.5 rounded bg-blue-500/15 text-blue-200 border border-blue-500/30 ak-blue">handle</span>
                            @elseif($row['kind'] === 'link')
                                <span class="text-xs px-2 py-0.5 rounded bg-sky-500/15 text-sky-200 border border-sky-500/30 ak-blue">primary alias</span>
                            @else
                                <span class="text-xs px-2 py-0.5 rounded bg-teal-500/15 text-teal-200 border border-teal-500/30 ak-green">extra alias</span>
                            @endif
                        </td>
                        <td class="px-5 py-3 font-mono text-white/90 ak-strong">{{ $row['label'] }}</td>
                        <td class="px-5 py-3 text-white/60 text-xs ak-muted">{{ $row['detail'] }}</td>
                        <td class="px-5 py-3 text-white/70 text-xs ak-strong">
                            @if($row['owner'])
                                <div>{{ $row['owner']->name ?: 'Unnamed' }}</div>
                                <div class="text-white/40 ak-note">{{ $row['owner']->email }}</div>
                            @else
                                ...
                                <div class="inline-flex items-center gap-2 flex-wrap justify-end">
                                    @if($row['kind'] === 'user' && $row['owner'])
                                        <form method="POST" action="{{ route('admin.banned-names.notify-user', [$item, $row['owner']->id]) }}" class="inline"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                    class="px-2.5 py-1.5 rounded-lg text-xs bg-blue-500/15 hover:bg-blue-500/25 text-blue-200 border border-blue-500/30 ak-blue">
                                    ...
                                    @if($row['acknowledged'])
                                        <form method="POST" action="{{ route('admin.banned-names.unacknowledge', $item) }}" class="inline">
                                            ...
                                            <button type="submit"
                                                    class="px-2.5 py-1.5 rounded-lg text-xs bg-white/5 hover:bg-white/10 text-white/60 border border-white/10 ak-muted"
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('admin.banned-names.acknowledge', $item) }}" class="inline">
                                            ...
                                            <input type="hidden" name="conflict_id" value="{{ $row['id'] }}">
                                            <button type="submit"
                                                    class="px-2.5 py-1.5 rounded-lg text-xs bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-200 border border-emerald-500/30 ak-green">

...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="space-y-3">
        @forelse($categories as $c)
            <form method="POST" action="{{ route('admin.blogs.categories.update', $c) }}" class="glass rounded-xl p-4 grid sm:grid-cols-12 gap-3 items-center">
                ...
                <div class="sm:col-span-2 flex justify-end gap-2">
                    <button class="px-3 py-2 bg-white/10 hover:bg-white/20 rounded text-xs text-white">Save</button>
                </div>
        ...
        @empty
            <div class="glass rounded-xl p-8 text-center text-white/50">No categories yet.</div>
        @endforelse
    </div>
</div>
@endsection

...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-white ak-strong">Tags</h1>
    ...
    <form method="POST" action="{{ route('admin.blogs.tags.store') }}" class="glass rounded-2xl p-5 grid sm:grid-cols-3 gap-3">
        @csrf
        <input type="text" name="name" required placeholder="Tag name" class="px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm ak-strong ak-input">
        ...
        @forelse($tags as $t)
            <div class="glass rounded-lg p-3 flex items-center gap-3">
                <form method="POST" action="{{ route('admin.blogs.tags.update', $t) }}" class="flex-1 flex gap-2">
                    @csrf @method('PUT')
                    <input type="text" name="name" value="{{ $t->name }}" class="flex-1 px-3 py-1.5 bg-white/5 border border-white/10 rounded text-white text-sm ak-strong ak-input">
                    <input type="text" name="slug" value="{{ $t->slug }}" class="flex-1 px-3 py-1.5 bg-white/5 border border-white/10 rounded text-white text-sm font-mono ak-strong ak-input">
                    <span class="text-xs text-white/50 self-center ak-muted">{{ $t->posts_count }}</span>
                    <button class="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded text-xs text-white ak-strong">Save</button>
                </form>
                <form method="POST" action="{{ route('admin.blogs.tags.destroy', $t) }}" onsubmit="return window.themedConfirmSubmit(this, {title: 'Delete this tag?', confirmText: 'Delete', confirmIcon: 'fa-trash', iconClass: 'fa-trash'})">
                    @csrf @method('DELETE')
                    <button class="px-3 py-1.5 bg-red-500/15 hover:bg-red-500/25 rounded text-xs text-red-300 ak-red">Delete</button>
                </form>
            </div>
        @empty
            <div class="glass rounded-xl p-8 text-center text-white/50 ak-muted">No tags yet.</div>
        @endforelse
    </div>

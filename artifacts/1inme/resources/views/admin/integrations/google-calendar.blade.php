...
@section('page-title', 'Google Calendar OAuth')
...
<div class="max-w-2xl space-y-6">
    ...
    <p class="ak-muted text-sm text-white/50">
        OAuth client credentials powering the Service Booking two-way Google Calendar sync &mdash; busy blocks
        imported from a connected calendar hide slots on public booking pages, and confirmed bookings are pushed
        as calendar events. The client secret is encrypted at rest and never displayed back &mdash; leave it blank
        to keep the stored value. Both the client ID and secret are required for the connect flow to turn on; until
        then the platform falls back to the
        <span class="font-mono">GOOGLE_CALENDAR_CLIENT_ID</span> / <span class="font-mono">GOOGLE_CALENDAR_CLIENT_SECRET</span>
        environment variables.
    </p>

    @include('admin.partials.help-note', [
        'body' => '<strong>How to create a Google OAuth client for Calendar sync</strong>
            <ol class="list-decimal pl-4 mt-1 space-y-0.5">
                ...
                <li>Enable the <strong>Google Calendar API</strong> under <a class="underline" href="https://console.cloud.google.com/apis/library" target="_blank" rel="noopener">APIs &amp; Services → Library</a>.</li>
                <li>Configure the <strong>OAuth consent screen</strong> (External or Internal) with the scopes listed below. If your app is External and not verified, add test users manually until Google verifies it.</li>
    ...
    <div class="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-xs space-y-1.5">
        ...
        <div class="flex flex-wrap gap-1.5">
            <code class="ak-strong px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-white/80 text-[11px] font-mono">https://www.googleapis.com/auth/calendar.events</code>
            <code class="ak-strong px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-white/80 text-[11px] font-mono">https://www.googleapis.com/auth/calendar.readonly</code>
            <code class="ak-strong px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-white/80 text-[11px] font-mono">https://www.googleapis.com/auth/userinfo.email</code>
        </div>
    </div>
    ...
    @include('admin.partials.copy-uri', [
        ...
        'value' => route('user.calendar.callback', ['provider' => 'google']),
    ])
    ...
    <form method="POST" action="{{ route('admin.integrations.google-calendar.update') }}" class="space-y-6">
        @csrf @method('PUT')

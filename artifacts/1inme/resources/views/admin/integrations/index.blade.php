...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-3xl">
        <div class="glass rounded-xl p-4 border border-white/10">
            <div class="text-[11px] uppercase tracking-wider text-white/40 flex items-center gap-2"><i class="fas fa-layer-group"></i> Integrations</div>
    ...
    @foreach($categories as $cat)
        <div class="space-y-3">
            <h2 class="text-sm font-semibold text-white/80 flex items-center gap-2">
                <i class="{{ $cat['icon'] }} text-white/40"></i> {{ $cat['label'] }}
            </h2>
                       ...
                       class="group glass rounded-2xl border {{ ($item['status']['tone'] ?? 'slate') === 'green' ? 'border-emerald-500/20' : (($item['status']['tone'] ?? 'slate') === 'amber' ? 'border-amber-500/20' : 'border-white/10') }} p-5 hover:bg-white/[0.03] transition flex flex-col gap-3">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white/80 bg-white/5 border border-white/10 shrink-0">
                                    <i class="{{ $item['icon'] }}"></i>
                                </div>
                                <div class="min-w-0">
                                    <div class="text-sm font-semibold text-white truncate">{{ $item['label'] }}</div>
                                    @if($item['external'])
                                        <div class="text-[10px] uppercase tracking-wider text-white/30 mt-0.5 flex items-center gap-1">

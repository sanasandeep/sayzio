...
<div class="max-w-2xl space-y-6">
    ...
    <form method="POST" action="{{ route('admin.integrations.google-places.update') }}" class="space-y-6">
        ...
        <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h3 class="ak-strong font-semibold text-white flex items-center gap-2">
                        <i class="ak-blue fab fa-google text-sky-400"></i> API key
                    </h3>
                    <p class="ak-note text-xs text-white/40">From Google Cloud Console &rarr; APIs &amp; Services &rarr; Credentials.</p>
                </div>
            ...
            <div>
                <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Google Places API key</label>
                @if($hasValue)
                ...
                <p class="ak-note text-[11px] text-white/30 mt-1">Encrypted at rest. Never displayed back.</p>
                @if($hasValue)

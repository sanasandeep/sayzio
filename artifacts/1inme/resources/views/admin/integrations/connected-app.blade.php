...
<div class="max-w-2xl space-y-6">
    ...
    @if($pm)
        @include('admin.partials.help-note', [
            'body' => '<strong>Setup steps for ' . $meta['label'] . '</strong>
                ...
                . ($pm['gotcha'] ? '<p class="mt-1.5 font-medium"><i class="ak-amber fas fa-triangle-exclamation text-amber-400 mr-1"></i>' . $pm['gotcha'] . '</p>' : ''),
        ])
    @else
        @include('admin.partials.help-note', [
    ...
    @if(count($oauthScopes))
        <div class="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-xs space-y-1.5">
            <div class="ak-note text-[10px] uppercase tracking-wider text-white/40">Required OAuth scopes, add these to your app</div>
            <div class="flex flex-wrap gap-1.5">
                @foreach($oauthScopes as $scope)
                    <code class="ak-strong px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-white/80 text-[11px] font-mono">{{ $scope }}</code>
                @endforeach
            </div>
        </div>
    @endif
        ...
        <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p class="ak-note text-xs text-white/40">From the {{ $meta['label'] }} developer console (Web / server OAuth app).</p>
                </div>
            ...
            <div>
                ...
                <p class="ak-note text-[11px] text-white/30 mt-1">Plain configuration (not a secret), safe to store unencrypted.</p>
            </div>

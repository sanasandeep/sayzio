...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="glass rounded-2xl p-6">
        <div class="flex items-start justify-between gap-4 mb-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white mb-1 ak-strong">App Launch Signups</h2>
        ...
        @unless($isLaunched)
            <div class="mb-4 px-3 py-2 bg-amber-500/10 border border-amber-400/30 text-amber-200 rounded-lg text-xs flex items-start gap-2 ak-amber">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
            <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                <div class="text-xs text-white/50 ak-muted">All signups</div>
                <div class="text-base font-semibold text-white ak-strong">{{ number_format($totals['all']) }}</div>
            </div>
            <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                <div class="text-xs text-white/50 ak-muted"><i class="fab fa-google-play mr-1 text-[10px]"></i>Google Play interest</div>
                <div class="text-base font-semibold text-white ak-strong">{{ number_format($totals['play']) }}</div>
            </div>
            <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                <div class="text-xs text-white/50 ak-muted"><i class="fab fa-apple mr-1 text-[10px]"></i>App Store interest</div>
                <div class="text-base font-semibold text-white ak-strong">{{ number_format($totals['app']) }}</div>
            </div>
        </div>
 ...
 -
                            ...
                            <td class="py-2 pr-3 text-xs">
                                @if($s->notified_at)
                                    <span class="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-200 ak-green">{{ $s->notified_at->format('Y-m-d') }}</span>
                                @else
                                    <span class="px-2 py-0.5 rounded-full bg-white/5 text-white/50 ak-muted">waiting</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr><td colspan="5" class="py-6 text-center text-white/40 text-sm ak-note">No signups yet, the form lives in the mobile-app coming-soon modal.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

...
<form method="POST" action="{{ route('admin.site-pages.update', $page->slug) }}"
    ...
    @method('PUT')
    <div>
        <h2 class="text-lg font-semibold text-white ak-strong">{{ $page->title }} <span class="text-xs text-white/40 ml-2 ak-note">/{{ $page->slug }}</span></h2>
        <p class="text-xs text-white/50 mt-1 ak-muted">Edit every category and feature row shown on the public Features page. Categories stay in the order listed below, nothing is collapsed or merged automatically.</p>
    </div>
    ...
    <div>
        ...
        <p class="text-[11px] text-white/40 mb-3 ak-note"><i class="fas fa-grip-vertical mr-1"></i> Tip: drag the handle on the left of any category or feature row to reorder. Arrow buttons still work too.</p>
            ...
            <div class="bg-white/5 border border-white/10 rounded-xl p-4 mb-4 space-y-3 transition-opacity"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] uppercase tracking-wider text-white/40 ak-note">Category <span x-text="ci+1"></span></span>
                    </div>
                    <div class="flex items-center gap-1">
                        <button type="button" @click="moveCategory(ci, -1)" :disabled="ci===0" class="text-xs text-white/60 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed px-2 py-1 ak-muted" title="Move up"><i class="fas fa-arrow-up"></i></button>
                        <button type="button" @click="moveCategory(ci, 1)" :disabled="ci===categories.length-1" class="text-xs text-white/60 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed px-2 py-1 ak-muted" title="Move down"><i class="fas fa-arrow-down"></i></button>
                        <button type="button" @click="removeCategory(ci)" class="text-xs text-red-400 hover:text-red-300 px-2 py-1 ak-red" title="Delete category"><i class="fas fa-trash"></i></button>
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div>
                        <label class="block text-[10px] uppercase tracking-wider text-white/40 mb-1 ak-note">Anchor id</label>
                    ...
                    <div>
                        <label class="block text-[10px] uppercase tracking-wider text-white/40 mb-1 ak-note">FontAwesome icon</label>
                        <div class="flex gap-1">
                            <input type="text" :name="'categories['+ci+'][icon]'" x-model="cat.icon" placeholder="fa-square-share-nodes"
                                   class="flex-1 min-w-0 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white ak-strong ak-input">
                ...
                <div>
                    <label class="block text-[10px] uppercase tracking-wider text-white/40 mb-1 ak-note">Intro</label>
                ...
                <div class="border-t border-white/10 pt-3">
                    <div class="flex items-center justify-between mb-2">
                        <label class="text-[10px] uppercase tracking-wider text-white/40 ak-note">Features in this category</label>
                        <div class="flex items-center gap-2">
                            <button type="button" x-show="cat.id === 'link-types' && homeSource.length > 0"
                                    @click="syncFromHome(ci)"
                                    class="text-[11px] px-2 py-1 bg-white/5 border border-white/10 hover:bg-white/10 rounded-lg text-white ak-strong"
                                    title="Replace these features with the current home-page link-types showcase">
                                <i class="fas fa-rotate mr-1"></i> Pull from Home
                            </button>
                            <button type="button" @click="addFeature(ci)" class="text-[11px] px-2 py-1 bg-white/5 border border-white/10 hover:bg-white/10 rounded-lg text-white ak-strong">
                                <i class="fas fa-plus mr-1"></i> Add feature
                            </button>
                        </div>
                    </div>
                    <p x-show="cat.id === 'link-types' && homeSource.length > 0" class="text-[11px] text-white/40 mb-2 ak-note">
                        ...
                        <div class="bg-black/20 border border-white/5 rounded-lg p-3 mb-2 space-y-2 transition-opacity"
                            ...
                            <div class="flex items-start gap-2">
                                <span draggable="true"
                                      ...
                                      class="cursor-grab active:cursor-grabbing text-white/40 hover:text-white/80 pt-1.5 select-none ak-note"
                                ...
                                <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2">
                                    <input type="text" :name="'categories['+ci+'][features]['+fi+'][name]'" x-model="feat.name" placeholder="Feature name"
                                           class="md:col-span-1 px-2 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white ak-strong ak-input">
                                    <textarea :name="'categories['+ci+'][features]['+fi+'][description]'" x-model="feat.description" rows="2" placeholder="Short description"
                                              class="md:col-span-2 px-2 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white ak-strong ak-input"></textarea>
                                    <input type="text" :name="'categories['+ci+'][features]['+fi+'][link]'" x-model="feat.link" placeholder="Optional link (e.g. /ai-chatbot or https://…)"
                                           class="md:col-span-3 px-2 py-1.5 bg-white/5 border border-white/10 rounded-lg text-[11px] text-white/80 ak-strong ak-input">
                                </div>
                                <div class="flex flex-col gap-1 shrink-0">
                                    <button type="button" @click="moveFeature(ci, fi, -1)" :disabled="fi===0" class="text-[11px] text-white/60 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed px-1.5 ak-muted" title="Move up"><i class="fas fa-arrow-up"></i></button>
                                    <button type="button" @click="moveFeature(ci, fi, 1)" :disabled="fi===cat.features.length-1" class="text-[11px] text-white/60 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed px-1.5 ak-muted" title="Move down"><i class="fas fa-arrow-down"></i></button>
                                    <button type="button" @click="removeFeature(ci, fi)" class="text-[11px] text-red-400 hover:text-red-300 px-1.5 ak-red" title="Delete feature"><i class="fas fa-trash"></i></button>
                                </div>
                            </div>
                        </div>
                    </template>
                    <div x-show="cat.features.length===0" class="text-[11px] text-white/40 text-center py-2 ak-note">No features in this category yet.</div>
                </div>
            </div>
        </template>

        <div x-show="categories.length===0" class="text-xs text-white/40 text-center py-4 ak-note">No categories yet, click "Add category".</div>
    </div>

    <div class="pt-4 border-t border-white/10 flex items-center justify-between">
        <a href="/{{ $page->slug }}" target="_blank" class="text-xs text-blue-400 hover:underline ak-blue">View live page <i class="fas fa-external-link-alt ml-1 text-[10px]"></i></a>

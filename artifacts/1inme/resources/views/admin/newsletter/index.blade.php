...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="glass rounded-2xl p-6">
        <div class="flex items-start justify-between gap-4 mb-4 flex-wrap">
            <div>
                <h2 class="text-lg font-semibold text-white mb-1 ak-strong">Newsletter Subscribers</h2>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.newsletter.compose') }}"
                   class="px-3 py-2 bg-emerald-500/15 border border-emerald-400/30 hover:bg-emerald-500/25 rounded-lg text-xs text-emerald-100 ak-green">
        ...
        <div class="mb-4 p-4 bg-white/[0.02] border border-white/10 rounded-xl">
            <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div>
                    <h3 class="text-sm font-semibold text-white ak-strong">Opt-out sources</h3>
                    <p class="text-[11px] text-white/40 ak-note">Last {{ $optOutBreakdown['window_days'] }} days · {{ number_format($optOutBreakdown['total']) }} unsubscribe{{ $optOutBreakdown['total'] === 1 ? '' : 's' }}</p>
                </div>
                <div class="flex items-center gap-3 text-[11px] text-white/60 ak-muted">
            ...
            @if($optOutBreakdown['total'] > 0)
                ...
                <div class="grid grid-cols-2 @if($optOutBreakdown['unknown']['count'] > 0) sm:grid-cols-3 @endif gap-3">
                    <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                        <div class="text-xs text-white/50 ak-muted">Inbox one-click</div>
                        <div class="text-base font-semibold text-white ak-strong">{{ $optOutBreakdown['inbox']['pct'] }}%
                            <span class="text-xs font-normal text-white/40 ak-note">· {{ number_format($optOutBreakdown['inbox']['count']) }}</span>
                        </div>
                    </div>
                    <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                        <div class="text-xs text-white/50 ak-muted">Footer link</div>
                        <div class="text-base font-semibold text-white ak-strong">{{ $optOutBreakdown['footer']['pct'] }}%
                            <span class="text-xs font-normal text-white/40 ak-note">· {{ number_format($optOutBreakdown['footer']['count']) }}</span>
                        </div>
                    </div>
                    @if($optOutBreakdown['unknown']['count'] > 0)
                        <div class="px-3 py-2 bg-white/[0.03] border border-white/5 rounded-lg">
                            <div class="text-xs text-white/50 ak-muted">Unknown</div>
                            <div class="text-base font-semibold text-white ak-strong">{{ $optOutBreakdown['unknown']['pct'] }}%
                                <span class="text-xs font-normal text-white/40 ak-note">· {{ number_format($optOutBreakdown['unknown']['count']) }}</span>
                            </div>
                        </div>
                    @endif
                </div>
            @else
                <p class="text-xs text-white/40 ak-note">No opt-outs in the last {{ $optOutBreakdown['window_days'] }} days.</p>
            @endif
        </div>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($subscribers as $s)
                        <tr class="text-white/80 ak-strong">
                            <td class="py-2 pr-3 font-mono text-xs text-white ak-strong">{{ $s->email }}</td>
                            <td class="py-2 pr-3 text-xs text-white/60 ak-muted">{{ $s->source ?: '—' }}</td>
                            <td class="py-2 pr-3 text-xs text-white/60 ak-muted">{{ optional($s->created_at)->format('Y-m-d H:i') }}</td>
                            <td class="py-2 pr-3 text-xs">
                                @if($s->unsubscribed_at)
                                    <div class="flex flex-col gap-0.5">
                                        <span class="px-2 py-0.5 rounded-full bg-white/5 text-white/50 inline-block w-fit ak-muted">
                                            unsubscribed {{ $s->unsubscribed_at->format('Y-m-d') }}
                                        </span>
                                        @if($s->unsubscribe_source === 'inbox')
                                            <span class="text-[10px] text-white/40 ak-note" title="One-click unsubscribe from the inbox provider (Gmail/Apple Mail) per RFC 8058">
                                                via inbox one-click
                                            </span>
                                        @elseif($s->unsubscribe_source === 'footer')
                                            <span class="text-[10px] text-white/40 ak-note" title="Recipient clicked the unsubscribe link in the email footer">
                                                via footer link
                                            </span>
                                        @else
                                            <span class="text-[10px] text-white/30 ak-note">source unknown</span>
                                        @endif
                                    </div>
                                @else
                                    <span class="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-200 ak-green">active</span>
                                @endif
                            </td>
                            <td class="py-2 pr-3 text-right">
                                <form method="POST" action="{{ route('admin.newsletter.destroy', $s) }}"
                                    ...
                                    @csrf @method('DELETE')
                                    <button class="text-xs text-red-300 hover:text-red-200 ak-red"><i class="fas fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="py-6 text-center text-white/40 text-sm ak-note">No subscribers yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

...
<div class="max-w-3xl space-y-6">
    ...
    @if (!empty($missingCredential))
        <div class="ak-amber p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs flex items-start gap-3">
            <i class="ak-amber fas fa-triangle-exclamation mt-0.5 text-amber-300"></i>
            <div class="space-y-1">
                <p class="ak-amber font-semibold text-amber-100">Outbound email is not configured to send.</p>
                <p>
    ...
    <form method="POST" action="{{ route('admin.mail-settings.update') }}" class="space-y-6"
        ...
        <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h3 class="ak-strong font-semibold text-white flex items-center gap-2">
                        <i class="ak-blue fas fa-server text-sky-400"></i> Mail transport
                    </h3>
                    <p class="ak-note text-xs text-white/40">Pick the mailer and, for SMTP, its connection details.</p>
                </div>
                <div class="shrink-0 flex flex-col items-end gap-1">
                    ...
                    @if($verifiedAt)
                        <span class="ak-green admin-mail-verified text-[11px] text-emerald-300/80 flex items-center gap-1"
                              title="Last successful SMTP handshake: {{ $verifiedAt->toDayDateTimeString() }}">
                            <i class="fas fa-check-circle"></i>
                            Verified OK {{ $verifiedAt->diffForHumans() }}
                        </span>
                    @else
                        <span class="ak-note text-[11px] text-white/30 flex items-center gap-1">
            ...
            <div>
                <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Mailer / driver</label>
                ...
                <p class="ak-note text-[11px] text-white/30 mt-1">Use <span class="font-mono">smtp</span> to send via a server. <span class="font-mono">log</span> writes mail to the log instead of sending.</p>
            </div>

            <div class="grid sm:grid-cols-2 gap-4" x-show="mailer === 'smtp'" x-cloak>
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">SMTP host</label>
                    <input type="text" name="host" value="{{ old('host', $host) }}" autocomplete="off"
                           placeholder="smtp.mailgun.org"
                           class="ak-strong ak-input w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">
                </div>
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">SMTP port</label>
                    <input type="number" name="port" value="{{ old('port', $port) }}" autocomplete="off"
                           placeholder="587" min="1" max="65535"
                           class="ak-strong ak-input w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">
                </div>
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Encryption</label>
                ...
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Username</label>
                    <input type="text" name="username" value="{{ old('username', $username) }}" autocomplete="off"
                           placeholder="postmaster@mg.example.com"
                           class="ak-strong ak-input w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">
                </div>
                <div class="sm:col-span-2">
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">Password</label>
                    @if($hasPassword)
                        <p class="ak-muted text-xs text-white/60 mb-1">Stored: <span class="ak-amber font-mono text-amber-300">{{ $maskedPassword }}</span></p>
                    @endif
            ...
            <p class="ak-note text-[11px] text-white/30">Password is encrypted at rest with the application key. Other fields are plain configuration.</p>
        </div>
        ...
        <div class="glass rounded-2xl border border-white/10 p-6 space-y-5">
            <div>
                <h3 class="ak-strong font-semibold text-white flex items-center gap-2">
                    <i class="ak-blue fas fa-id-badge text-blue-400"></i> Default &ldquo;from&rdquo; identity
                </h3>
                <p class="ak-note text-xs text-white/40">The sender shown on every outgoing email.</p>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">From address</label>
                    <input type="email" name="from_address" value="{{ old('from_address', $fromAddress) }}" autocomplete="off"
                           placeholder="hello@1in.me"
                           class="ak-strong ak-input w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">
                </div>
                <div>
                    <label class="ak-note text-xs uppercase tracking-wider text-white/40 mb-1 block">From name</label>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <span class="ak-note text-xs text-white/30">Saving runs a connection check automatically.</span>
        </div>
    </form>
    ...
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-3">
        <h3 class="ak-strong font-semibold text-white text-sm flex items-center gap-2">
            <i class="ak-amber fas fa-plug text-amber-400"></i> Verify connection
        </h3>
    ...
    {{-- ============================================================ --}}
    <div class="glass rounded-2xl border border-white/10 p-6 space-y-3">
        <h3 class="ak-strong font-semibold text-white text-sm">Send a test email</h3>
        <p class="ak-note text-xs text-white/40">Sends a sample message through the saved transport and reports the result.</p>
        <form method="POST" action="{{ route('admin.mail-settings.test') }}" class="flex gap-2">
            ...
            <input type="email" name="test_email" required placeholder="you@example.com"
                   class="ak-strong ak-input flex-1 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">

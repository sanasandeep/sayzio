...
<div class="max-w-4xl mx-auto space-y-6">
    <div>
        <h1 class="text-xl font-semibold text-white">Public announcements</h1>
        ...
        @foreach($audiences as $audienceKey => $audienceLabel)
            ...
            <div class="glass rounded-2xl p-6 space-y-4">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-base font-semibold text-white">{{ $audienceLabel }}</h2>
                        <p class="text-xs text-white/50 mt-0.5">{{ $audienceHints[$audienceKey] ?? '' }}</p>
                    </div>
                    <label class="inline-flex items-center gap-2 text-xs font-medium text-white/70 shrink-0 cursor-pointer">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Link URL <span class="text-white/30 normal-case font-normal">(optional)</span></label>
                        ...
                        @error("announcements.$audienceKey.link_url")<p class="mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Link label <span class="text-white/30 normal-case font-normal">(optional)</span></label>
                        <input type="text" name="announcements[{{ $audienceKey }}][link_label]" maxlength="60"
                               ...
                               class="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                        @error("announcements.$audienceKey.link_label")<p class="mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
        @endforeach

...
<div class="glass rounded-2xl p-6 space-y-3">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="ak-strong text-lg font-semibold text-white">{{ $title }}</h2>
            <p class="ak-muted text-xs text-white/50">{{ $helper }}</p>
        </div>
    ...
    <template x-for="(t,i) in {{ $modelKey }}" :key="i">
        <div class="bg-white/5 border border-white/10 rounded-xl p-4 space-y-2">
            <div class="flex items-center justify-between">
                <span class="ak-note text-[10px] uppercase tracking-wider text-white/40">Testimonial <span x-text="i+1"></span></span>
                <button type="button" @click="{{ $modelKey }}.splice(i,1)" class="ak-red text-red-400 hover:text-red-300 text-xs"><i class="fas fa-trash"></i></button>
            </div>
            <textarea :name="'{{ $fieldName }}['+i+'][quote]'" x-model="t.quote" rows="3" placeholder="What they said about Sayzio…"
                      class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white"></textarea>
    ...
    <p x-show="{{ $modelKey }}.length===0" class="ak-note text-xs text-white/40">No testimonials yet, add one to show the section publicly.</p>
                ...
                <template x-for="(t,i) in {{ $modelKey }}" :key="'{{ $modelKey }}p'+i">
                    <figure x-show="(t.quote || '').trim() !== ''"
                        ...
                        <figcaption class="mt-3 flex items-center gap-2.5">
                            ...
                            <template x-if="(t.photo || '').trim() === ''">
                                <div class="ak-strong w-8 h-8 rounded-full text-white text-xs font-bold flex items-center justify-center"

...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('admin.marketing-settings.update') }}"
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            <div>
                <h2 class="ak-strong text-lg font-semibold text-white">Tracking &amp; share preview</h2>
                <p class="ak-muted text-xs text-white/50">Injected only on marketing pages (landing, features, how-it-works, faqs, about, contact, services, policies). Dashboard, biolinks and short-link redirects are never touched.</p>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Google Analytics 4, Measurement ID</label>
                    <input type="text" name="ga4_id" value="{{ old('ga4_id', $ga4_id) }}" placeholder="G-XXXXXXX"
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-mono">
                    @error('ga4_id')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Meta (Facebook) Pixel ID</label>
                    <input type="text" name="meta_pixel_id" value="{{ old('meta_pixel_id', $meta_pixel_id) }}" placeholder="1234567890"
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-mono">
                    @error('meta_pixel_id')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
            </div>
            <div>
                <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Default share preview image (URL)</label>
                ...
                <p class="ak-note mt-1 text-[11px] text-white/40">Used as the default Open Graph / Twitter Card image when a page does not specify its own. 1200×630 PNG/JPG recommended.</p>
                @error('default_share_image')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            <div>
                <h2 class="ak-strong text-lg font-semibold text-white">Mobile app store links</h2>
                <p class="ak-muted text-xs text-white/50">Power the Google Play / App Store buttons in the homepage Dialer section and the public footer. Leave a field blank and its button shows a "coming soon" modal instead of linking out.</p>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Google Play URL</label>
                    <input type="url" name="play_store_url" value="{{ old('play_store_url', $play_store_url) }}"
                           placeholder="https://play.google.com/store/apps/details?id=..."
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    @error('play_store_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Apple App Store URL</label>
                    <input type="url" name="app_store_url" value="{{ old('app_store_url', $app_store_url) }}"
                           placeholder="https://apps.apple.com/app/id..."
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    @error('app_store_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            ...
            <div class="grid sm:grid-cols-3 gap-4">
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Chrome Web Store URL</label>
                    <input type="url" name="extension_chrome_url" value="{{ old('extension_chrome_url', $extension_chrome_url) }}"
                           placeholder="https://chromewebstore.google.com/detail/..."
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    @error('extension_chrome_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Edge Add-ons URL</label>
                    <input type="url" name="extension_edge_url" value="{{ old('extension_edge_url', $extension_edge_url) }}"
                           ...
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    @error('extension_edge_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Firefox Add-ons URL</label>
                    <input type="url" name="extension_firefox_url" value="{{ old('extension_firefox_url', $extension_firefox_url) }}"
                           ...
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    @error('extension_firefox_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="url" name="browser_linux_appimage_url" value="{{ old('browser_linux_appimage_url', $browser_linux_appimage_url) }}"
                ...
                <div>
                    ...
                    <input type="url" name="browser_linux_deb_url" value="{{ old('browser_linux_deb_url', $browser_linux_deb_url) }}"
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            <div>
                <h2 class="ak-strong text-lg font-semibold text-white">3-way Subscribe block, WhatsApp</h2>
                <p class="ak-muted text-xs text-white/50">Powers the WhatsApp Channel and WhatsApp DM cards in the public Subscribe block (every marketing page) and the compact version in the site footer. Leave a field blank to hide that card.</p>
            </div>
            <div>
                <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">WhatsApp Channel URL</label>
                <input type="url" name="whatsapp_channel_url" value="{{ old('whatsapp_channel_url', $whatsapp_channel_url) }}"
                       ...
                       class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                <p class="ak-note mt-1 text-[11px] text-white/40">Public invite link to the brand's WhatsApp Channel. When empty, the "WhatsApp Channel" card is hidden across the site.</p>
                @error('whatsapp_channel_url')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">WhatsApp DM number (E.164)</label>
                    <input type="text" name="whatsapp_number" value="{{ old('whatsapp_number', $whatsapp_number) }}"
                           placeholder="+15551234567"
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white font-mono">
                    <p class="ak-note mt-1 text-[11px] text-white/40">Country code + number, no spaces. Used to build the wa.me link for the "Chat on WhatsApp" card.</p>
                    @error('whatsapp_number')<p class="ak-red mt-1 text-xs text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="ak-muted block text-xs font-semibold uppercase tracking-wider text-white/60 mb-1.5">Default opening message</label>
                    <input type="text" name="whatsapp_message" value="{{ old('whatsapp_message', $whatsapp_message) }}"
                           placeholder="Hi Sayzio! I'd like to learn more."
                           class="ak-strong ak-input w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
        ...
        <div class="glass rounded-2xl p-6 space-y-3">
            ...
            <template x-for="(t,i) in trust" :key="i">
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 grid sm:grid-cols-[1fr_2fr_1fr_auto] gap-2 items-center">
                    ...
                    <button type="button" @click="trust.splice(i,1)" class="ak-red text-red-400 hover:text-red-300 text-xs px-2"><i class="fas fa-trash"></i></button>
                </div>
            </template>
            <p x-show="trust.length===0" class="ak-note text-xs text-white/40">No metrics yet, add at least one to show the strip.</p>
        ...
        <div class="glass rounded-2xl p-6 space-y-3">
            ...
            <div class="ak-note hidden sm:grid grid-cols-[2fr_1fr_1fr_auto] gap-2 px-1 text-[11px] font-bold uppercase tracking-wider text-white/40">
                <div>Feature</div>
                <div>Sayzio</div>
                <div>Competitor</div>
                <div></div>
            ...
            <template x-for="(w,i) in why" :key="i">
                <div class="bg-white/5 border border-white/10 rounded-xl p-3 grid sm:grid-cols-[2fr_1fr_1fr_auto] gap-2 items-center">
                    ...
                    <input type="text" :name="'why_comparison['+i+'][theirs]'" x-model="w.theirs" placeholder="Limited"
                           class="ak-strong ak-input px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white">
                    <button type="button" @click="why.splice(i,1)" class="ak-red text-red-400 hover:text-red-300 text-xs px-2"><i class="fas fa-trash"></i></button>
                </div>
            </template>
            <p x-show="why.length===0" class="ak-note text-xs text-white/40">No rows yet, the comparison section will be hidden on the landing page.</p>

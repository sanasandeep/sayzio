        ...
        <table class="w-full text-sm">
            ...
            @forelse($stats as $stat)
                <tr class="hover:bg-white/[0.03]">
                    <td class="px-4 py-3 text-gray-400 ak-muted">{{ $stat->sort_order }}</td>
                    <td class="px-4 py-3">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: linear-gradient(135deg, {{ $stat->color }}, #3d6bff);">
                            <i class="fas {{ $stat->icon }} text-white ak-strong"></i>
                        </div>
                    </td>
                    <td class="px-4 py-3 font-bold text-white ak-strong">{{ $stat->value }}<span class="text-cyan-300 ak-blue">{{ $stat->suffix }}</span></td>
                    <td class="px-4 py-3 text-gray-200 ak-strong">{{ $stat->label }}</td>
                    <td class="px-4 py-3"><span class="inline-flex items-center gap-2 text-xs text-gray-300 ak-muted"><span class="w-4 h-4 rounded" style="background: {{ $stat->color }}"></span>{{ $stat->color }}</span></td>
                    <td class="px-4 py-3">
                        <form method="POST" action="{{ route('admin.site-stats.toggle', $stat) }}">@csrf
                            <button class="text-xs px-2 py-1 rounded-full {{ $stat->is_active ? 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 ak-green' : 'bg-gray-500/15 text-gray-400 border border-gray-500/30 ak-muted' }}">{{ $stat->is_active ? 'Active' : 'Hidden' }}</button>
                        </form>
                    </td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="{{ route('admin.site-stats.edit', $stat) }}" class="text-cyan-300 hover:text-cyan-200 mr-3 ak-blue"><i class="fas fa-pen"></i></a>
                        <form method="POST" action="{{ route('admin.site-stats.destroy', $stat) }}" class="inline" onsubmit="event.preventDefault(); themedConfirmAsync({title:'Delete stat?',body:'This cannot be undone.',confirmText:'Delete',variant:'danger'}).then(ok=>{ if(ok) this.submit(); });">@csrf @method('DELETE')
                            <button class="text-rose-300 hover:text-rose-200 ak-red"><i class="fas fa-trash"></i></button>
                        </form>
                    </td>
                </tr>
            @empty
                <tr><td colspan="7" class="px-4 py-10 text-center text-gray-500">No stats yet. <a href="{{ route('admin.site-stats.create') }}" class="text-cyan-300 ak-blue">Add your first stat</a>.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>

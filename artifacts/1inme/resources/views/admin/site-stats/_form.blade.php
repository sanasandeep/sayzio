...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div>
        ...
        <input type="color" name="color" value="{{ old('color', $stat->color ?? '#7c3aed') }}" class="w-full h-10 rounded-lg bg-white/5 border border-white/10">
    </div>
    <div>
...
<div class="mt-6 flex items-center gap-3">
    <button class="px-5 py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-violet-500 text-white text-sm font-semibold hover:opacity-90">{{ ($stat ?? null)?->exists ? 'Save changes' : 'Add stat' }}</button>

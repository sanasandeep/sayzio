...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <p class="text-[11px] text-white/40 mt-1.5 ak-note">
            Stored on events as <code class="text-white/60 ak-muted">settings.event_category</code>. Changing the slug on an
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
    <div>
        <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5 ak-muted">
            Icon <span class="text-white/30 normal-case ak-note">(Font Awesome class)</span>
        </label>
    ...
    <div>
        <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5 ak-muted">Gradient start</label>
        <input type="text" name="color_from" value="{{ old('color_from', $category->color_from ?? '#3d6bff') }}"
               required maxlength="20"
               class="w-full bg-black/30 border border-white/15 rounded-lg px-3 py-2 text-sm text-white font-mono ak-strong">
    </div>
    <div>
        <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5 ak-muted">Gradient end</label>
...
<div class="flex items-center gap-3">
    <div class="rounded-xl overflow-hidden border border-white/10 w-16 h-16 flex items-center justify-center text-white text-xl ak-strong"
    ...
    <label class="flex items-center gap-2 cursor-pointer px-3 h-[42px] rounded-lg bg-black/30 border border-white/15">
        ...
        <input type="checkbox" name="is_enabled" value="1"
               ...
               class="rounded">
        <span class="text-sm text-white/80 ak-strong">Enabled &amp; shown on /events</span>
    </label>
</div>

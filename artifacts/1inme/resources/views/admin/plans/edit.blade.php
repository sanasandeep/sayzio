...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-dark-200 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.plans.update', $plan) }}">
            ...
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-medium text-dark-700 mb-1">Plan Name</label>
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="w-full px-4 py-2.5 border border-dark-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">{{ old('description', $plan->description) }}</textarea>
                </div>

                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-dark-700 mb-1">Monthly Price ($)</label>
                        <input type="number" name="monthly_price" value="{{ old('monthly_price', $plan->monthly_price) }}" step="0.01" min="0" required
                               class="w-full px-4 py-2.5 border border-dark-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-dark-700 mb-1">Annual Price ($)</label>
                        <input type="number" name="annual_price" value="{{ old('annual_price', $plan->annual_price) }}" step="0.01" min="0" required
                               class="w-full px-4 py-2.5 border border-dark-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-dark-700 mb-1">Trial Days</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-dark-700 mb-1">Sort Order</label>
                ...
                <div class="border-t border-dark-200 pt-5">
                    <h3 class="text-sm font-medium text-dark-700 mb-3">Plan Features / Limits</h3>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs text-dark-500 mb-1">Max Links</label>
                            <input type="number" name="features[max_links]" value="{{ $features['max_links'] ?? 10 }}" min="0"
                                   class="w-full px-3 py-2 border border-dark-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-xs text-dark-500 mb-1">Max Biolinks</label>
                            <input type="number" name="features[max_biolinks]" value="{{ $features['max_biolinks'] ?? 1 }}" min="0"
                                   class="w-full px-3 py-2 border border-dark-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-xs text-dark-500 mb-1">Max File Size (MB)</label>
                            <input type="number" name="features[max_file_size_mb]" value="{{ $features['max_file_size_mb'] ?? 5 }}" min="0"
                                   class="w-full px-3 py-2 border border-dark-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-xs text-dark-500 mb-1">Max Projects</label>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
                        ...
                        <label class="flex items-center gap-2 text-sm text-dark-600 p-2 rounded hover:bg-dark-50">
                            <input type="checkbox" name="features[{{ $key }}]" value="1"
                                   ...
                                   class="rounded border-dark-300 text-primary-600">
                            {{ $label }}
                        </label>
                        @endforeach
                    </div>
                ...
                <div class="flex items-center gap-3 pt-4">
                    <button type="submit" class="px-6 py-2.5 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 transition">Update Plan</button>
                    <a href="{{ route('admin.plans.index') }}" class="px-6 py-2.5 bg-dark-100 text-dark-700 rounded-lg font-medium hover:bg-dark-200 transition">Cancel</a>
                </div>
            </div>
        </form>
    </div>

...
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-white/40 ak-note">Manage subscription plans and pricing</p>
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.plans.export') }}" class="px-4 py-2 bg-white/10 text-white/80 rounded-xl text-sm font-medium hover:bg-white/20 transition ak-strong">
            <i class="fas fa-download mr-2"></i>Export CSV
        </a>
        <button type="button" @click="importOpen = true" class="px-4 py-2 bg-white/10 text-white/80 rounded-xl text-sm font-medium hover:bg-white/20 transition ak-strong">
...
<div class="mb-4 flex items-center gap-2 text-xs text-white/40 ak-note" x-show="!anySelected()">
    <i class="fas fa-check-square"></i>
    Tick the checkbox on 2–6 plan cards to open a side-by-side Compare &amp; Edit view.
</div>
<div class="mb-4 flex items-center gap-2 text-xs text-blue-300 ak-blue" x-show="anySelected()" x-cloak>
    <i class="fas fa-check-square"></i>
    <span x-text="selectedCount() + ' plan' + (selectedCount() === 1 ? '' : 's') + ' selected'"></span>
    <span class="text-blue-300/50 mx-1 ak-blue">·</span>
    <button type="button" @click="clearAll()" class="text-white/40 hover:text-white/70 underline underline-offset-2 ak-note">Clear</button>
</div>
...
<div x-show="importOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div class="relative w-full max-w-lg glass rounded-2xl border border-white/10 p-6">
        <div class="flex items-start justify-between mb-4">
            <div>
                <h2 class="text-lg font-semibold text-white/90 ak-strong">Import plan changes</h2>
                <p class="text-xs text-white/50 mt-1 leading-relaxed ak-muted">
                    Upload a CSV in the same format as the export. Rows are matched to
                    plans by their <span class="font-mono text-white/70 ak-strong">Slug</span>, and you'll
                    see a diff of every change before anything is saved.
                </p>
            </div>
            <button type="button" @click="importOpen = false" class="text-white/40 hover:text-white/80 ak-note">
        ...
        <form method="POST" action="{{ route('admin.plans.import.preview') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-[11px] uppercase tracking-wider text-white/50 font-bold mb-1.5 ak-muted">
                    CSV file <span class="text-white/30 normal-case ak-note">(max 4 MB)</span>
                </label>
                ...
                <p class="text-[11px] text-white/40 mt-2 ak-note">
                    Tip: click <span class="text-white/60 ak-muted">Export CSV</span> first, edit the values,
            ...
            <div class="flex items-center gap-3 pt-1">
                ...
                <button type="button" @click="importOpen = false" class="text-sm text-white/60 hover:text-white ak-muted">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<div x-data="{ open: false }" class="mt-10">
    <button type="button" @click="open = !open" class="flex items-center gap-2 text-sm text-white/50 hover:text-white/80 transition ak-muted">
        <i class="fas" :class="open ? 'fa-chevron-down' : 'fa-chevron-right'"></i>
        Import history ({{ $imports->count() }})
        <span class="text-white/30 ak-note"> - undo a recent CSV import if the numbers came out wrong</span>
    </button>
    <div x-show="open" x-cloak class="mt-4 glass rounded-2xl border border-white/10 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="text-[11px] uppercase tracking-wider text-white/40 bg-white/5 ak-note">
                <tr>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="text-white/70 ak-strong">
                    <td class="px-4 py-3 whitespace-nowrap">{{ $import->created_at?->format('M j, Y g:i A') }}</td>
                    <td class="px-4 py-3 text-white/60 ak-muted">{{ $import->admin_name ?? '—' }}</td>
                    <td class="px-4 py-3">
                        <span class="text-white/80 ak-strong">{{ $import->plans_updated }} plan(s)</span>
                        @if(!empty($import->changed))
                        <span class="text-white/40 text-xs block mt-0.5 ak-note">
                            {{ collect($import->changed)->pluck('name')->filter()->take(6)->implode(', ') }}{{ count($import->changed) > 6 ? '…' : '' }}
                        </span>
                        @endif
                        @if($import->rows_skipped > 0)
                        <span class="text-white/30 text-xs ak-note">({{ $import->rows_skipped }} row(s) skipped)</span>
                        @endif
                    </td>
                    <td class="px-4 py-3">
                        @if($import->reverted_at)
                        <span class="inline-flex items-center gap-1.5 text-xs px-2 py-1 rounded-lg bg-white/5 text-white/40 ak-note">
                            <i class="fas fa-rotate-left"></i>
                            Reverted{{ $import->reverted_by_name ? ' by ' . $import->reverted_by_name : '' }}
                        </span>
                        @else
                        <span class="inline-flex items-center gap-1.5 text-xs px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-200 ak-green">
                    ...
                    <td class="px-4 py-3 text-right">
                        ...
                        <form method="POST" action="{{ route('admin.plans.import.revert', $import) }}"
                            ...
                            @csrf
                            <button type="submit" class="px-3 py-1.5 rounded-lg text-xs font-medium bg-rose-500/15 hover:bg-rose-500/25 text-rose-200 transition ak-red">
...
<div x-data="{ open: false }" class="mt-10">
    <button type="button" @click="open = !open" class="flex items-center gap-2 text-sm text-white/50 hover:text-white/80 transition ak-muted">
        ...
        <span class="text-white/30 ak-note"> - legacy plans kept for existing subscribers</span>
    </button>
...
<div x-show="selectedCount() >= 2" x-cloak class="compare-float-bar" x-transition>
    ...
    <span x-show="selectedCount() > 6" class="text-xs text-amber-300 ak-amber">Max 6</span>
    <button type="button" @click="clearAll()" class="text-white/40 hover:text-white/70 ml-1 text-xs ak-note" title="Clear selection">
        <i class="fas fa-times"></i>
    </button>
</div>

...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="glass rounded-2xl p-6">
        ...
        <form method="POST" action="{{ route('admin.branding.update') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-[260px,1fr] gap-4 items-center">
                ...
                <div>
                    <label class="block text-sm font-medium text-white mb-1 ak-strong">Wordmark, light mode</label>
                    <p class="text-xs text-white/50 mb-2 ak-muted">Shown on light backgrounds. Use the dark/colored version of your logo.</p>
                    ...
                    @error('logo_light')<p class="mt-1 text-xs text-red-400 ak-red">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-[260px,1fr] gap-4 items-center">
                ...
                <div>
                    <label class="block text-sm font-medium text-white mb-1 ak-strong">Wordmark, dark mode</label>
                    <p class="text-xs text-white/50 mb-2 ak-muted">Shown on dark backgrounds. Use the white/light version of your logo.</p>
                    <input type="file" name="logo_dark" accept="image/png,image/jpeg,image/webp,image/svg+xml"
                           class="block w-full text-xs text-white/70 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:cursor-pointer">
                    @error('logo_dark')<p class="mt-1 text-xs text-red-400 ak-red">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-[260px,1fr] gap-4 items-center">
                ...
                <div>
                    <label class="block text-sm font-medium text-white mb-1 ak-strong">Square icon / favicon</label>
                    <p class="text-xs text-white/50 mb-2 ak-muted">Used as the favicon, app icon, and small badges.</p>
                    <input type="file" name="icon" accept="image/png,image/jpeg,image/webp,image/x-icon"
                           class="block w-full text-xs text-white/70 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:cursor-pointer">
                    @error('icon')<p class="mt-1 text-xs text-red-400 ak-red">{{ $message }}</p>@enderror
                </div>
            </div>
    ...
    <div class="glass rounded-2xl p-6">
        <h3 class="text-sm font-semibold text-white mb-1 ak-strong">Reset to defaults</h3>
        <p class="text-xs text-white/50 mb-3 ak-muted">Restores the bundled Sayzio logos.</p>

...
<body class="flex flex-col items-center {{ $mbEnabled && $mbPos === 'bottom' ? 'min-h-screen' : '' }}">
    ...
    (function() {
        ...
        if (editBlockId || params.get('_preview')) {
            ...
            // place for known block types/fields and ack back whether EVERY
            // changed field was handled — only then does the editor skip the
            // full iframe reload after autosave. Anything we don't recognise
            // acks handled=false so the safe reload fallback stays intact.
            (function () {
                ...
                window.addEventListener('message', function (e) {
                    ...
                    var root = document.querySelector('[data-block-id="' + d.blockId + '"]');
                    var handled = false;
                    if (root && Array.isArray(d.changed) && d.fields) {

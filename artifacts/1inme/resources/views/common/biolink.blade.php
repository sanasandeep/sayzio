...
<body class="flex flex-col items-center {{ $mbEnabled && $mbPos === 'bottom' ? 'min-h-screen' : '' }}">
    ...
    <div class="biolink-container">
        ...
        {{-- The block loop is a partial so the page types that share the
             block EDITOR can also share the RENDERER. Menus could add
             blocks and never show them, because this loop only ever
             existed here. See common/partials/biolink-block-list. --}}
        @include('common.partials.biolink-block-list', [
            'link'           => $link,
            'blkFontColor'   => $fontColor ?? '#ffffff',
            'blkGlobalTheme' => $globalTheme ?? [],
            'blkBtnInline'   => $btnInline ?? '',
        ])
    ...
    (function() {
        ...
        // The instant-patch channel below must be active whenever the page is
        // hosted inside the editor's preview iframe. That iframe loads the
        // signed owner preview URL (`?_preview=1`) — `_editBlock` is only
        // present on the explicit scroll-to-block deep link — so gate on
        // either param.
    ...
    {{-- Instant block live preview (Task #4022 / #4034) — extracted into a
         shared partial so the slides public page can reuse the exact same
         listener + handlers. Gated internally on ?_preview / ?_editBlock. --}}

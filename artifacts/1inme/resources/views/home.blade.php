...
<style>
    ...
    .dark .spy-strip{
        background:linear-gradient(180deg,rgba(10,10,20,.78) 0%,rgba(10,10,20,.62) 100%);
        border-bottom-color:rgba(255,255,255,.06);
    }
    .spy-strip::before{
        content:"";position:absolute;inset:auto 0 -1px 0;height:1px;
        background:linear-gradient(90deg,transparent 0,rgba(34,211,238,.5) 18%,rgba(139,92,246,.55) 50%,rgba(244,114,182,.5) 82%,transparent 100%);
        opacity:.55;
    }
    .spy-eyebrow{
    ...
    .spy-link{
        ...
        padding:7px 13px;border-radius:9999px;font-size:12.5px;font-weight:600;
        color:#475569;white-space:nowrap;
        transition:color .18s ease, background-color .18s ease, transform .18s ease;
    }
    .dark .spy-link{color:#94a3b8;}
    .spy-link:hover{color:#0f172a;background:rgba(15,23,42,.05);}
    .dark .spy-link:hover{color:#fff;background:rgba(255,255,255,.06);}
    .spy-link.is-active{
        color:#fff;
        background:linear-gradient(90deg,#22d3ee,#6366f1 50%,#ec4899);
        box-shadow:0 6px 18px -6px rgba(99,102,241,.55), inset 0 0 0 1px rgba(255,255,255,.18);
    }
    .spy-link.is-active::after{
    ...
    .spy-fade{
        pointer-events:none;position:absolute;top:0;bottom:0;width:32px;
    }
    .spy-fade-l{left:0;background:linear-gradient(90deg,rgba(255,255,255,.95),transparent);}
    .spy-fade-r{right:0;background:linear-gradient(-90deg,rgba(255,255,255,.95),transparent);}
    .dark .spy-fade-l{background:linear-gradient(90deg,rgba(10,10,20,.85),transparent);}
    .dark .spy-fade-r{background:linear-gradient(-90deg,rgba(10,10,20,.85),transparent);}
</style>
<div x-data="homeScrollSpy()" x-init="init()" class="spy-strip hidden md:block">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex items-center gap-3">
            <span class="spy-eyebrow flex items-center gap-1.5 shrink-0 py-2.5">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="color:#8b5cf6"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
                On this page
            </span>
            <div class="relative flex-1 min-w-0">
                <nav class="flex items-center gap-1 overflow-x-auto py-2 [scrollbar-width:none] [-ms-overflow-style:none] [&amp;::-webkit-scrollbar]:hidden"
                    ...
                    @foreach([
                        ...
                        <a href="#{{ $id }}"
                           ...
                           class="spy-link shrink-0">{{ $lbl }}</a>
                    @endforeach
                </nav>
                <span class="spy-fade spy-fade-l" aria-hidden="true"></span>
                <span class="spy-fade spy-fade-r" aria-hidden="true"></span>
            </div>
        </div>
    </div>
</div>

...
const ALIAS = `e2e-blk-bgsw-${Date.now().toString(36)}${process.pid.toString(36)}`;
...
/**
 * Seed the demo user plus a biolink with a heading block that has a custom
 * style (bg_color) but NO catalog preset yet, so picking a swatch exercises
 * the live layer-creation path (Task #5990).
 */
...
$bio = Link::create([
  ...
  'alias' => '${ALIAS}', 'title' => 'E2E Block BG Preset Swatch Live', 'is_active' => true,
]);
$h = BiolinkBlock::create([
  'link_id' => $bio->id, 'type' => 'heading', 'sort_order' => 0, 'is_active' => true,
  'settings' => [
    'text' => 'Preset Swatch Live',
    '_style' => ['bg_color' => '#101828'],
  ],
]);
...
test("clicking a preset swatch paints the block layer live, clicking again removes it — no reload", async ({
  page,
}) => {
  ...
  // Style panel is collapsed behind "Block Styling"; the preset picker lives
  // in the Look tab, behind the "Browse" toggle.
  ...
  // Arm the autosave wait BEFORE the click so the debounced POST can't race.
  ...
  // The preset layer appears via the live channel — before/without any save
  // round-trip completing — carrying the gradient_zero catalog CSS.
  await expect(layer).toBeAttached({ timeout: 10_000 });
  const bgImage = await layer.evaluate(
  ...
  // Debounced autosave still persists the picked key.
  await autosave;
  const saved = runTinkerSeed(
    `echo 'KEY=' . (App\\Modules\\User\\Models\\BiolinkBlock::find(${ids.headingId})->settings['_style']['bg_preset_key'] ?? 'missing');`,
  );
  expect(saved).toContain("KEY=gradient_zero");

  // Clicking the selected swatch again removes the preset live (the block
  // has a .block-styled wrapper, so removal is non-structural).
  ...
  await autosave2;
  const cleared = runTinkerSeed(
    `echo 'KEY=' . (App\\Modules\\User\\Models\\BiolinkBlock::find(${ids.headingId})->settings['_style']['bg_preset_key'] ?? 'cleared');`,
  );
  expect(cleared).toContain("KEY=cleared");
});

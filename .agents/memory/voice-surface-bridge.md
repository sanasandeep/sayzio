...
**navigate_to deferral differs by platform:** web defers the nav until the TTS `<audio>` `ended` event fires (true "speech first"); mobile defers via a *fixed short timer* (`setTimeout(... , 600)` in `sendClip`) while audio plays independently through expo-audio. Same user-visible intent (reply/speech gets a beat before nav), different mechanism — don't assume mobile waits on audio end. Regression-guarded by `1inme-mobile` `test:voice-bridge` (source-driven: lifts the real `sendClip` + bus + `mapNavTarget`), mirroring the web `voice-assistant-bridge.spec.ts`.

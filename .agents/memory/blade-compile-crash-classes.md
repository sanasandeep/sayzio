...
- Attached directive `word@if(...)...@endif` — `@if` glued to a preceding word fails the `\B` boundary, orphaning `@endif`. → Replace the whole optional fragment with an inline expression: `{{ $cond ? ' ...' : '' }}` (or `{!! ... e($x) ... !!}` when it contains HTML). See also blade-endif-if-gotcha.md.

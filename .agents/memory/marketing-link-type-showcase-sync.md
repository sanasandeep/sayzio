...
- The `features` row's `meta_description` (item 3 above) is itself a **separately-seeded, drift-prone value**: it's baked in once (by `SitePagesSeeder`/a first-run `firstOrCreate`) and never re-synced from code, so a live row can lag the current count by several versions even when the code default is correct. Detect drift by regex (e.g. `^Everything you get with Sayzio — all \d+ link types`) rather than an exact old-string match, and overwrite only when it still matches that seeded pattern (skip if an admin's edit no longer matches).

...
- **npm ci "Exit handler never called" on the EAS builder**: crashed deterministically with the dialer's old package-lock.json (made by a different npm major); fix = `rm package-lock.json && npm install --package-lock-only --ignore-scripts` locally (npm 11) and rebuild. Symptoms downstream: `expo: not found` in PREBUILD / `expo config` exit 1 — root cause is the failed install, not the config.

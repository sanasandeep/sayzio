...
**Backdrop snapshots need a generation token.** A `capturePage` snapshot used as a menu backdrop is async; if the overlay is released (or re-acquired) before delivery, a stale image lands over fresh UI. Manager keeps `overlayGeneration` (bumped on first acquire) and delivers the capture only while `overlayCount>0 && generation` matches. Also: any backdrop element rendered in App.tsx must appear in ALL mode branches (browser/dashboard/split), not just the one it was developed in — early returns silently drop it.
...
**Split-tab divider drag + per-pane backdrops.** Two-native-pane tab splits (e.g. Website+Website) can't get live mousemove over native views — the divider drag holds the chrome overlay (views detach, backdrop snapshots show), tracks the ratio in renderer state, and commits via IPC only on mouseup (main re-lays-out on release). The overlay backdrop payload is an ARRAY of {dataUrl,bounds} — one snapshot per visible native pane — or split tabs show a blank pane behind open menus/panels.

...
## Plan-cap counts are account-wide
Owner plan-cap re-checks (`max_links`/`max_biolinks`, e.g. wizard finish/finishAi + API generate) must use `links()->withoutGlobalScope('workspace')` — the BelongsToWorkspace scope otherwise hides links outside the active workspace and the cap silently never trips in web requests. Same applies to test assertions counting Link rows after an in-process web request (the request binds current_workspace and poisons later queries in the test process).
**Why:** factory-provisioned default workspaces made workspace-less fixture links invisible; in prod, workspace-scoped counting would let users exceed plan caps by switching workspaces.

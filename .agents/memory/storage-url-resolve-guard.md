...
Rule: any payload builder under `artifacts/1inme/app/Modules/{Api,Common,User}` emitting a storage-backed image column (avatar, image, cover_image, logo, favicon, og_image, banner, photo, thumbnail) must wrap it in `\App\Support\PublicStorageUrl::resolve()`.

...
**"Get directions" handoff:** alongside `#ev-map`, both surfaces render a `<a target="_blank" href="https://www.google.com/maps/dir/?api=1&destination={lat},{lng}">` (built once as `$directionsUrl` in the `@php` gate). On desktop/mobile browsers this opens Google Maps / the native maps app. But the RSVP page is shown on mobile inside `EmbedModal`'s WebView with `setSupportMultipleWindows={false}`, so `target="_blank"` loads in the same webview and fires `onShouldStartLoadWithRequest` — EmbedModal intercepts maps/`tel:`/`mailto:`/`geo:` URLs there and hands them to `Linking.openURL` (return false), letting all other same-flow navigation proceed. Add new external-app handoffs to that regex, not per-page JS.

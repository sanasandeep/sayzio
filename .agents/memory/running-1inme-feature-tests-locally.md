...
**AI-builder / wizard-flow Feature tests can't run on stale helium either:** any test that drives the full `generate()`/wizard controller path (e.g. `BiolinkWizardAiCreditTest`, `BiolinkAiBuilderResourcesTest`) hits a recently-added column/table helium lacks. The first failing query is SWALLOWED by an app try/catch, so phpunit only shows a cascade `SQLSTATE[25P02] current transaction is aborted` on a later innocent query (`plans`, `app_settings`). Proof it's environmental, not your test: the known-CI-green sibling `BiolinkWizardAiCreditTest` fails the SAME way on helium. For these, trust CI — don't chase the cascade.

...
export default function AuthLanding() {
  ...
  const onSendOtp = async () => {
    ...
    if (__DEV__ && !getConfiguredBaseUrl()) {
      setError(
        "API base URL isn't set. Add EXPO_PUBLIC_API_BASE_URL to your .env so OTP doesn't go to production.",
      );
      return;
    }
  ...
  const onSocial = async (provider: SocialProvider) => {
    ...
    if (__DEV__ && !getConfiguredBaseUrl()) {
      const msg =
        "API base URL isn't set. Add EXPO_PUBLIC_API_BASE_URL to your .env so OAuth doesn't redirect through production.";
      if (Platform.OS === "web") setError(msg);
      else showAlert("Sign in", msg);
      return;
    }
